...
bitmap('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_n_loci.png', units='px', width=3000, height=2000, res=300)
...
bitmap('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_n_sig_loci.png', units='px', width=3000, height=2000, res=300)
...
bitmap('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_n_sig_fdr_loci.png', units='px', width=3000, height=2000, res=300)
...
bitmap('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_rho.png', units='px', width=3000, height=3000, res=300)
...
bitmap('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_z.png', units='px', width=3000, height=3000, res=300)
...
#########
# Using DEPR06 and INTE03, compare results with and without restricting to HapMap3
#########

target_gwas_i<-'DEPR06'
secondary_gwas_i<-'INTE03'

univ_res<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/',target_gwas_i,'/',target_gwas_i,'.',secondary_gwas_i,'.univ.lava'))
univ_res_nonhm3<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_nonhm3/',target_gwas_i,'/',target_gwas_i,'.',secondary_gwas_i,'.univ.lava'))

univ_res_both<-merge(univ_res[,c('locus','phen','n.snps','n.pcs','h2.obs', 'p'), with=F],univ_res_nonhm3[,c('locus','phen','n.snps','n.pcs','h2.obs', 'p'), with=F], by=c('locus','phen'), all=T)

sum(!is.na(univ_res_both$h2.obs.x[univ_res_both$phen == 'DEPR06']))
sum(!is.na(univ_res_both$h2.obs.x[univ_res_both$phen == 'INTE03']))
sum(!is.na(univ_res_both$h2.obs.y[univ_res_both$phen == 'DEPR06']))
sum(!is.na(univ_res_both$h2.obs.y[univ_res_both$phen == 'INTE03']))

sum(univ_res_both$p.x[univ_res_both$phen == 'DEPR06'] < 0.05, na.rm=T)
sum(univ_res_both$p.x[univ_res_both$phen == 'INTE03'] < 0.05, na.rm=T)
sum(univ_res_both$p.y[univ_res_both$phen == 'DEPR06'] < 0.05, na.rm=T)
sum(univ_res_both$p.y[univ_res_both$phen == 'INTE03'] < 0.05, na.rm=T)

cor(univ_res_both$h2.obs.x, univ_res_both$h2.obs.y, use='p')

# Many more loci have nominally significant h2 when not restricting to hm3 SNPs.

bivar_res<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/',target_gwas_i,'/',target_gwas_i,'.',secondary_gwas_i,'.bivar.lava'))
bivar_res_nonhm3<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_nonhm3/',target_gwas_i,'/',target_gwas_i,'.',secondary_gwas_i,'.bivar.lava'))

bivar_res_both<-merge(bivar_res[,c('locus','n.snps', 'rho','rho.lower', 'rho.upper', 'p'), with=F],bivar_res_nonhm3[,c('locus','n.snps', 'rho','rho.lower', 'rho.upper', 'p'), with=F], by=c('locus'), all=T)

sum(!is.na(bivar_res_both$rho.x))
sum(!is.na(bivar_res_both$rho.y))

sum(bivar_res_both$p.x < 0.05, na.rm=T)
sum(bivar_res_both$p.y < 0.05, na.rm=T)

cor(bivar_res_both$rho.x, bivar_res_both$rho.y, use='p')

# Similarly, many more loci have nominally significant rho when not restricting to hapmap3 variants.
# h2 and rho estimates are moderately correlated (0.79 for both).

# To me this suggests, not restricting to hapmap3 variants does improve power, and gwas sumstat imputation could further increase power.
# These results should be use in the PGS calculation to test for improvements.
...
for(i in 1:length(target_gwas)){
  ...
  for(type in c('rho_restricted_unweighted','rho_fdr_restricted_unweighted','h2_restricted_unweighted','rho_restricted_rho_weighted','rho_fdr_restricted_rho_weighted','h2_restricted_rho_weighted','rho_restricted_rho_h2_weighted','rho_fdr_restricted_rho_h2_weighted','h2_restricted_rho_h2_weighted')){
    ...
    if(grepl('h2_restricted',type)){
      tmp_eval$Model<-gsub('hsq-p < 0.05\n','',tmp_eval$Model)  
      tmp_eval$Model<-factor(tmp_eval$Model, levels=tmp_eval$Model)
