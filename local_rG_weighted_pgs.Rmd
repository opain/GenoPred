...
### Not restricted to hm3

#### Sanity check

Run LDSC using the GenoFunc prepared sumstats. The results should be identical. If they are not there is cause for concern.
...
for gwas in $(echo DEPR06 SCHI02 BIPO02 AUTI07 SMOK04 ADHD05 COLL01 DIAB05 ANXI02 MENA01F OBES01 WAIS01 COAD01 GLYC05 INTE01 INTE03 BODY04); do
  sbatch -p brc,shared ~/brc_scratch/Software/munge_sumstats.sh \
   --sumstats /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/${gwas}.cleaned.gz \
   --merge-alleles /users/k1806347/brc_scratch/Data/ldsc/w_hm3.snplist \
   --out /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/${gwas}.cleaned
done

# Run bivariate LDSC for all traits
mkdir /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/sample_overlap_nonhm3

for gwas1 in $(echo DEPR06 SCHI02 BIPO02 AUTI07 SMOK04 ADHD05 COLL01 DIAB05 ANXI02 MENA01F OBES01 WAIS01 COAD01 GLYC05 INTE01 INTE03 BODY04); do
for gwas2 in $(echo DEPR06 SCHI02 BIPO02 AUTI07 SMOK04 ADHD05 COLL01 DIAB05 ANXI02 MENA01F OBES01 WAIS01 COAD01 GLYC05 INTE01 INTE03 BODY04); do
   sbatch -p brc,shared ~/brc_scratch/Software/ldsc.sh \
     --rg /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/${gwas1}.cleaned.sumstats.gz,/users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/${gwas2}.cleaned.sumstats.gz \
     --ref-ld-chr /users/k1806347/brc_scratch/Data/ldsc/eur_w_ld_chr/ \
     --w-ld-chr /users/k1806347/brc_scratch/Data/ldsc/eur_w_ld_chr/ \
     --out /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/sample_overlap_nonhm3/${gwas1}_${gwas2}_rg
done
done

```

Try runnning LAVA using summary statistics that are not retricted to HapMap3 variants as this may increase power.

```{bash, eval=F, echo=T}
# Use GenoFunc sumstat_cleaner to format and QC the GWAS without restricting to hm3 SNPs
. /users/k1806347/brc_scratch/Software/MyGit/GenoPred/config_used/Pipeline_prep.config

# Create directory
mkdir -p /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3

# Create file listing GWAS that haven't been processed.
> /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/todo.txt
for gwas in $(echo DEPR06 SCHI02 BIPO02 AUTI07 SMOK04 ADHD05 COLL01 DIAB05 ANXI02 MENA01F OBES01 WAIS01 COAD01 GLYC05 INTE01 INTE03 BODY04);do
if [ ! -f /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/${gwas}.cleaned.gz ]; then
echo $gwas >> /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/todo.txt
fi
done

# Create shell script to run using sbatch
cat > /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/sbatch.sh << 'EOF'
#!/bin/sh
...
gwas=$(sed "${SLURM_ARRAY_TASK_ID}q;d" /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/todo.txt)
echo ${gwas}

/users/k1806347/brc_scratch/Software/Rscript.sh /mnt/lustre/users/k1806347/Software/MyGit/GenoFunc/GenoFuncPipe/scripts/sumstat_cleaner.R \
  --sumstats ${gwas_rep_cleaned}/${gwas}.gz \
  --ref_chr /mnt/lustre/users/k1806347/Software/MyGit/GenoFunc/GenoFuncPipe/resources/data/1kg/1KG.Phase3.EUR.MAF_001.chr \
  --output /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/${gwas}.cleaned

EOF

sbatch --array 1-$(wc -l /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/todo.txt | cut -d' ' -f1)%5 /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/sbatch.sh

# GLYC05 contains MAF so FREQ doesn't correspond to particular allele. As a result we are loosing many SNPs. Remove FREQ column and re-QC.

cp ${gwas_rep_cleaned}/GLYC05.gz /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/GLYC05_tmp.gz
zcat /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/GLYC05_tmp.gz | cut -f 1-3,5-8 | gzip > /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/GLYC05_tmp2.gz

/users/k1806347/brc_scratch/Software/Rscript.sh /mnt/lustre/users/k1806347/Software/MyGit/GenoFunc/GenoFuncPipe/scripts/sumstat_cleaner.R \
  --sumstats /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/GLYC05_tmp2.gz \
  --ref_chr /mnt/lustre/users/k1806347/Software/MyGit/GenoFunc/GenoFuncPipe/resources/data/1kg/1KG.Phase3.EUR.MAF_001.chr \
  --output /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/GLYC05.cleaned
  
rm /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/GLYC05_tmp.gz
rm /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/GLYC05_tmp2.gz
...
pheno<-data.frame(phenotype=gwas,
                  ...
                  controls=NA,
                  prevalence=NA,
                  filename=paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/', gwas,'.cleaned.gz'))

dir.create('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/')
write.table(pheno, '/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/input_nohm3.info.txt', col.names=T, row.names=F, quote=F)
...
# Run LAVA for each target GWAS
########
# Use MAGMA EUR reference as it is not restricted to HapMap3

for target_gwas in $(echo DEPR06 BODY04 INTE03); do
for secondary_gwas in $(echo DEPR06 SCHI02 BIPO02 AUTI07 SMOK04 ADHD05 COLL01 DIAB05 ANXI02 MENA01F OBES01 WAIS01 COAD01 GLYC05 INTE01 INTE03 BODY04); do
if [ ${target_gwas} != ${secondary_gwas} ]; then
if [ ! -f /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_nonhm3/${target_gwas}/${target_gwas}.${secondary_gwas}.bivar.lava ]; then
mkdir -p /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_nonhm3/${target_gwas}
sbatch -p brc,shared --mem 10G /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/lava.R \
  /users/k1806347/brc_scratch/Data/1KG/MAGMA/g1000_eur \
  /users/k1806347/brc_scratch/Data/LAVA/blocks_s2500_m25_f1_w200.GRCh37_hg19.locfile \
  /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/input_nohm3.info.txt \
  /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/sample_overlap/sample.overlap.txt \
  ${target_gwas},${secondary_gwas} \
  /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_nonhm3/${target_gwas}/${target_gwas}.${secondary_gwas}
fi
fi
done
done

# This takes approximately 12-18 hours per pair of traits
```
...
Try runnning LAVA using summary statistics that have been imputed. 

```{bash, eval=F, echo=T}
# Impute GWAS summary statistics that have been QC'd by the GenoFunc sumstat_cleaner script
# Run Imputation using FIZI without incorporating functional annotations.
. /users/k1806347/brc_scratch/Software/MyGit/GenoPred/config_used/Pipeline_prep.config

# Create directory
mkdir -p /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed

# Create file listing GWAS that haven't been processed.
> /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed/todo.txt
for gwas in $(echo DEPR06 SCHI02 BIPO02 AUTI07 SMOK04 ADHD05 COLL01 DIAB05 ANXI02 MENA01F OBES01 WAIS01 COAD01 GLYC05 INTE01 INTE03 BODY04);do
if [ ! -f /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed/${gwas}.imputed.gz ]; then
echo $gwas >> /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed/todo.txt
fi
done

# Create shell script to run using sbatch
cat > /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed/sbatch.sh << 'EOF'
#!/bin/sh
...
gwas=$(sed "${SLURM_ARRAY_TASK_ID}q;d" /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed/todo.txt)
echo ${gwas}
...
/users/k1806347/brc_scratch/Software/Rscript.sh /mnt/lustre/users/k1806347/Software/MyGit/GenoFunc/GenoFuncPipe/scripts/sumstat_imputer.R \
  --sumstats /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_nonhm3/${gwas}.cleaned.gz \
  --ref_plink_chr /scratch/groups/biomarkers-brc-mh/Reference_data/1KG_Phase3/PLINK/EUR/EUR_phase3.MAF_001.chr \
  --fizi /users/k1806347/brc_scratch/Software/fizi/bin/fizi \
  --output /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed/${gwas} \
  --n_cores 10
  
EOF

sbatch --array 1-$(wc -l /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed/todo.txt | cut -d' ' -f1)%3 /users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed/sbatch.sh
...
pheno<-data.frame(phenotype=gwas,
                  ...
                  controls=NA,
                  prevalence=NA,
                  filename=paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/GWAS_sumstats_imputed/', gwas,'.imputed.gz'))

dir.create('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/')
write.table(pheno, '/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/input_imputed.info.txt', col.names=T, row.names=F, quote=F)
...
for secondary_gwas in $(echo DEPR06 SCHI02 BIPO02 AUTI07 SMOK04 ADHD05 COLL01 DIAB05 ANXI02 MENA01F OBES01 WAIS01 COAD01 GLYC05 INTE01 INTE03 BODY04); do
if [ ${target_gwas} != ${secondary_gwas} ]; then
if [ ! -f /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_imputed/${target_gwas}/${target_gwas}.${secondary_gwas}.bivar.lava ]; then
mkdir -p /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_imputed/${target_gwas}
sbatch -p brc,shared --mem 10G /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/lava.R \
  /users/k1806347/brc_scratch/Data/1KG/MAGMA/g1000_eur \
  /users/k1806347/brc_scratch/Data/LAVA/blocks_s2500_m25_f1_w200.GRCh37_hg19.locfile \
  /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/input_imputed.info.txt \
  /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/sample_overlap/sample.overlap.txt \
  ${target_gwas},${secondary_gwas} \
  /users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_imputed/${target_gwas}/${target_gwas}.${secondary_gwas}
fi
fi
done
done
...
Now run the same code when not resticting to hapmap3 SNPs.
...
for(target_gwas_i in target_gwas){
  for(secondary_gwas_i in secondary_gwas[secondary_gwas != target_gwas_i]){
    univ_res<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_nonhm3/',target_gwas_i,'/',target_gwas_i,'.',secondary_gwas_i,'.univ.lava'))
    bivar_res<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results_nonhm3/',target_gwas_i,'/',target_gwas_i,'.',secondary_gwas_i,'.bivar.lava'))
    ...
    # Subset loci that are significant after multiple testing
    univ_res_wide_h2_sig<-univ_res_wide[univ_res_wide$p.x < (0.05) & univ_res_wide$p.y < (0.05),]
    
    # Subset bicar results to contain only loci with significant SNP-h2
    bivar_res<-bivar_res[bivar_res$locus %in% univ_res_wide_h2_sig$locus,]
    bivar_res<-bivar_res[!is.na(bivar_res$p),]
...
write.table(des, paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/lava_descript_nonhm3.csv'), row.names = F, quote=F)
write.table(all_res, paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/all_res_nonhm3.csv'), row.names = F, quote=F)

#########
# Compare results without and without restriction to HapMap3 variants
#########

des_hm3<-fread('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/lava_descript.csv')
des_nonhm3<-fread('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/lava_descript_nonhm3.csv')

des_hm3<-des_hm3[,c('target','secondary','n_loci','n_sig_loci','n_sig_fdr_loci'),with=F]
des_nonhm3<-des_nonhm3[,c('target','secondary','n_loci','n_sig_loci','n_sig_fdr_loci'),with=F]

des_both<-merge(des_hm3, des_nonhm3, by=c('target','secondary'))
des_both$Test<-paste0(des_both$target,'-',des_both$secondary)
des_both<-melt(des_both)
des_both$Group<-NA
des_both$Group[grepl('.x', des_both$variable)]<-'hm3'
des_both$Group[grepl('.y', des_both$variable)]<-'unrestricted'
  
library(ggplot2)
library(cowplot)

png('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_n_loci.png', units='px', width=3000, height=2000, res=300)
ggplot(des_both[grepl('n_loci', des_both$variable),], aes(x=Test, y=value, fill=Group)) +
...
png('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_n_sig_loci.png', units='px', width=3000, height=2000, res=300)
ggplot(des_both[grepl('n_sig_loci', des_both$variable),], aes(x=Test, y=value, fill=Group)) +
  geom_bar(stat="identity", position=position_dodge()) +
  theme_half_open() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x='', y='N loci with rG.p < 0.05')
dev.off()

png('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_n_sig_fdr_loci.png', units='px', width=3000, height=2000, res=300)
ggplot(des_both[grepl('n_sig_fdr_loci', des_both$variable),], aes(x=Test, y=value, fill=Group)) +
  geom_bar(stat="identity", position=position_dodge()) +
  theme_half_open() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x='', y='N loci with rG.fdr < 0.05')
dev.off()

# Note. There are more loci analysed when unresitrcted, indicating power to detect significant SNP-h2 increased
# There are also typically more loci with nominally significant rG, but when applying FDR correction, there typically any many less significant loci when unrestricted.
# There are less surviving multiple testing because there are more loci tested due to increased power.

# My interpretation of this is that the p-values of the rG are biased when the number of SNPs available is lower.

all_res_hm3<-fread('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/all_res.csv')
all_res_nonhm3<-fread('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/all_res_nonhm3.csv')

all_res_hm3$Test<-paste0(all_res_hm3$phen1,'-',all_res_hm3$phen2)
all_res_nonhm3$Test<-paste0(all_res_nonhm3$phen1,'-',all_res_nonhm3$phen2)

all_res_both<-merge(all_res_hm3[,c('locus','rho','p','phen1','phen2')], all_res_nonhm3[,c('locus','rho','p','phen1','phen2')], by=c('locus','phen1','phen2'))

all_res_both$z.x<-abs(qnorm(all_res_both$p.x/2))
all_res_both$z.y<-abs(qnorm(all_res_both$p.y/2))

png('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_rho.png', units='px', width=3000, height=3000, res=300)
ggplot(all_res_both, aes(x=rho.x, y=rho.y)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, colour='red', size=2) +
  coord_fixed() +
  labs(x="rho (hm3)",y="rho (unrestricted)") +
  theme_half_open() +
  xlim(c(-1,1)) +
  ylim(c(-1,1))
dev.off()

png('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hm3_vs_unrestricted_z.png', units='px', width=3000, height=3000, res=300)
ggplot(all_res_both, aes(x=z.x, y=z.y)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, colour='red', size=2) +
  coord_fixed() +
  labs(x="rho.z (hm3)",y="rho.z (unrestricted)") +
  theme_half_open() +
  xlim(c(0,max(c(all_res_both$z.x, all_res_both$z.y)))) +
  ylim(c(0,max(c(all_res_both$z.x, all_res_both$z.y))))

dev.off()

# This shows clearly that there is a strong correlation between rho and the significant of rho. However, the magnitude of rho and the Z are stronger when restricted to HapMap3 SNPs.
...
for(target_gwas_i in target_gwas){
  for(secondary_gwas_i in secondary_gwas[secondary_gwas != target_gwas_i]){
    ...
    fwrite(score[,c('SNP','A1','SCORE'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.score.gz"), quote=F, sep=' ', na='NA')
    ...
    # Write out sig h2 restricted but unweighted
    fwrite(score_bim_reweight[,c('SNP','A1','SCORE'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.h2_restricted_unweighted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out sig rho restricted but unweighted
    fwrite(score_bim_reweight[,c('SNP','A1','SCORE_sig'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_restricted_unweighted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out fdr sig rho restricted but unweighted
    fwrite(score_bim_reweight[,c('SNP','A1','SCORE_fdr'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_fdr_restricted_unweighted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out sig h2 restricted and rho weighted
    fwrite(score_bim_reweight[,c('SNP','A1','SCORE_rho'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.h2_restricted_rho_weighted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out sig rho restricted and rho weighted
    fwrite(score_bim_reweight[,c('SNP','A1','SCORE_rho_sig'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_restricted_rho_weighted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out fdr sig rho restricted and rho weighted
    fwrite(score_bim_reweight[,c('SNP','A1','SCORE_rho_fdr'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_fdr_restricted_rho_weighted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out sig h2 restricted and rho and h2 weighted
    fwrite(score_bim_reweight[,c('SNP','A1','SCORE_rho_h2'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.h2_restricted_rho_h2_weighted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out sig rho restricted and rho and h2 weighted
    fwrite(score_bim_reweight[,c('SNP','A1','SCORE_rho_sig_h2'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_restricted_rho_h2_weighted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out fdr rho restricted and rho and h2 weighted
    fwrite(score_bim_reweight[,c('SNP','A1','SCORE_rho_fdr_h2'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_fdr_restricted_rho_h2_weighted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')
    ...
    # Write out sig h2 restricted and rho weighted with unadjusted SNPs
    fwrite(score_full_reweight[,c('SNP','A1','SCORE_rho'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.h2_restricted_rho_weighted_with_unadjusted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out sig rho restricted and rho weighted with unadjusted SNPs
    fwrite(score_full_reweight[,c('SNP','A1','SCORE_rho_sig'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_restricted_rho_weighted_with_unadjusted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out fdr sig rho restricted and rho weighted with unadjusted SNPs
    fwrite(score_full_reweight[,c('SNP','A1','SCORE_rho_fdr'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_fdr_restricted_rho_weighted_with_unadjusted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out sig h2 restricted and rho and h2 weighted with unadjusted SNPs
    fwrite(score_full_reweight[,c('SNP','A1','SCORE_rho_h2'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.h2_restricted_rho_h2_weighted_with_unadjusted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out sig rho restricted and rho and h2 weighted with unadjusted SNPs
    fwrite(score_full_reweight[,c('SNP','A1','SCORE_rho_sig_h2'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_restricted_rho_h2_weighted_with_unadjusted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out fdr rho restricted and rho and h2 weighted with unadjusted SNPs
    fwrite(score_full_reweight[,c('SNP','A1','SCORE_rho_fdr_h2'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_fdr_restricted_rho_h2_weighted_with_unadjusted_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')
    ...
    # Write out unadjusted for SNPs not in sig h2 loci
    fwrite(score_unadjust[,c('SNP','A1','SCORE_remainder'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.h2_restricted_remainder_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out unadjusted for SNPs not in sig rho loci
    fwrite(score_unadjust[,c('SNP','A1','SCORE_sig_remainder'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_restricted_remainder_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')

    # Write out unadjusted for SNPs not in fdr sig rho loci
    fwrite(score_unadjust[,c('SNP','A1','SCORE_fdr_remainder'),with=F], paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,".pseudo.nonhm3.rho_fdr_restricted_remainder_",target_gwas_i,'.score.gz'), quote=F, sep=' ', na='NA')
...
for(target_gwas_i in target_gwas){
  for(secondary_gwas_i in secondary_gwas[secondary_gwas != target_gwas_i]){
    for(type in c('rho_restricted_unweighted','rho_fdr_restricted_unweighted','h2_restricted_unweighted','rho_restricted_rho_weighted','rho_fdr_restricted_rho_weighted','h2_restricted_rho_weighted','rho_restricted_rho_h2_weighted','rho_fdr_restricted_rho_h2_weighted','h2_restricted_rho_h2_weighted','rho_restricted_rho_weighted_with_unadjusted','rho_fdr_restricted_rho_weighted_with_unadjusted','h2_restricted_rho_weighted_with_unadjusted','rho_restricted_rho_h2_weighted_with_unadjusted','rho_fdr_restricted_rho_h2_weighted_with_unadjusted','h2_restricted_rho_h2_weighted_with_unadjusted','rho_restricted_remainder','rho_fdr_restricted_remainder','h2_restricted_remainder')){
      dir.create(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/processed/',secondary_gwas_i,'.pseudo.nonhm3.',type,'_',target_gwas_i), recursive=T)
        if(!file.exists(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/processed/',secondary_gwas_i,'.pseudo.nonhm3.',type,'_',target_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,'.pseudo.nonhm3.',type,'_',target_gwas_i,'.EUR.scale'))){
          system(paste0('sbatch -p brc,shared --mem 10G  /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/external_score_processor/external_score_processor_plink2.R --ref_plink_chr /users/k1806347/brc_scratch/Data/1KG/Phase3/1KGPhase3.w_hm3.chr --score /users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,'.pseudo.nonhm3.',type,'_',target_gwas_i,'.score.gz --plink2 /users/k1806347/brc_scratch/Software/plink2 --output /users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/processed/',secondary_gwas_i,'.pseudo.nonhm3.',type,'_',target_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,'.pseudo.nonhm3.',type,'_',target_gwas_i,' --ref_pop_scale /users/k1806347/brc_scratch/Data/1KG/Phase3/super_pop_keep.list'))
      }
    }
  }
}
...
for(i in 1:length(target_gwas)){
  ...
  for(secondary_gwas_i in secondary_gwas[secondary_gwas != target_gwas_i]){
    for(type in c('rho_restricted_unweighted','rho_fdr_restricted_unweighted','h2_restricted_unweighted','rho_restricted_rho_weighted','rho_fdr_restricted_rho_weighted','h2_restricted_rho_weighted','rho_restricted_rho_h2_weighted','rho_fdr_restricted_rho_h2_weighted','h2_restricted_rho_h2_weighted','rho_restricted_rho_weighted_with_unadjusted','rho_fdr_restricted_rho_weighted_with_unadjusted','h2_restricted_rho_weighted_with_unadjusted','rho_restricted_rho_h2_weighted_with_unadjusted','rho_fdr_restricted_rho_h2_weighted_with_unadjusted','h2_restricted_rho_h2_weighted_with_unadjusted','rho_restricted_remainder','rho_fdr_restricted_remainder','h2_restricted_remainder')){
      if(!file.exists(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/prs_nonhm3/',secondary_gwas_i,'/',target_gwas_i,'/',type,'/prs.profiles'))){
        system(paste0('sbatch -p brc,shared --mem 10G /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/Scaled_polygenic_scorer/Scaled_polygenic_scorer_plink2.R --target_plink_chr /users/k1806347/brc_scratch/Data/UKBB/Genotype/Harmonised/UKBB.w_hm3.QCd.AllSNP.chr --target_keep /users/k1806347/brc_scratch/Data/UKBB/Phenotype/PRS_comp_subset/UKBB.',target_pheno_i,'.txt --ref_score /users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,'.pseudo.nonhm3.',type,'_',target_gwas_i,'.score.gz --ref_scale /users/k1806347/brc_scratch/Analyses/local_rg_pgs/score_files/LDAK/',secondary_gwas_i,'/processed/',secondary_gwas_i,'.pseudo.nonhm3.',type,'_',target_gwas_i,'/1KGPhase3.w_hm3.',secondary_gwas_i,'.pseudo.nonhm3.',type,'_',target_gwas_i,'.EUR.scale --ref_freq_chr /users/k1806347/brc_scratch/Data/1KG/Phase3/freq_files/EUR/1KGPhase3.w_hm3.EUR.chr --plink2 /users/k1806347/brc_scratch/Software/plink2_dev/plink2 --output /users/k1806347/brc_scratch/Analyses/local_rg_pgs/prs_nonhm3/',secondary_gwas_i,'/',target_gwas_i,'/',type,'/prs'))
      }
    }
  }
  Sys.sleep(6000)
...
for(i in 1:length(target_gwas)){
  ...
  for(secondary_gwas_i in secondary_gwas[secondary_gwas != target_gwas_i]){
    for(type in c('rho_restricted_unweighted','rho_fdr_restricted_unweighted','h2_restricted_unweighted','rho_restricted_rho_weighted','rho_fdr_restricted_rho_weighted','h2_restricted_rho_weighted','rho_restricted_rho_h2_weighted','rho_fdr_restricted_rho_h2_weighted','h2_restricted_rho_h2_weighted')){
      # Add in prs for target phenotype
      tmp<-data.frame(predictors=paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/prs_nonhm3/',secondary_gwas_i,'/',target_gwas_i,'/',type,'/prs.profiles'),
                      group=type)
  ...
  dir.create(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i), recursive=T)

  write.table(pred_file, paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_2.txt'), col.names=T, row.names=F, quote=F)

  system(paste0('sbatch --mem 10G -p brc,shared /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/Model_builder/Model_builder_V2_nested.R --pheno /users/k1806347/brc_scratch/Data/UKBB/Phenotype/PRS_comp_subset/UKBB.',target_pheno_i,'.txt --out /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_2 --assoc T --outcome_pop_prev ',target_pop_prev_i,' --predictors /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_2.txt'))
...
for(i in 1:length(target_gwas)){
  for(type in c('rho_restricted_unweighted','rho_fdr_restricted_unweighted','h2_restricted_unweighted','rho_restricted_rho_weighted','rho_fdr_restricted_rho_weighted','h2_restricted_rho_weighted','rho_restricted_rho_h2_weighted','rho_fdr_restricted_rho_h2_weighted','h2_restricted_rho_h2_weighted')){
    ...
    write.table(pred_file, paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_3_',type,'.txt'), col.names=T, row.names=F, quote=F)
  
    system(paste0('sbatch --mem 10G -p brc,shared /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/Model_builder/Model_builder_V2_nested.R --pheno /users/k1806347/brc_scratch/Data/UKBB/Phenotype/PRS_comp_subset/UKBB.',target_pheno_i,'.txt --out /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_3_',type,' --assoc T --outcome_pop_prev ',target_pop_prev_i,' --predictors /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_3_',type,'.txt'))
...
for(i in 1:length(target_gwas)){
    ...
    write.table(pred_file, paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_4_',type,'.txt'), col.names=T, row.names=F, quote=F)
  
    system(paste0('sbatch --mem 10G -p brc,shared /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/Model_builder/Model_builder_V2_nested.R --pheno /users/k1806347/brc_scratch/Data/UKBB/Phenotype/PRS_comp_subset/UKBB.',target_pheno_i,'.txt --out /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_4_',type,' --assoc T --outcome_pop_prev ',target_pop_prev_i,' --predictors /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_4_',type,'.txt'))
...
for(i in 1:length(target_gwas)){
    ...
    write.table(pred_file, paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_5_',type,'.txt'), col.names=T, row.names=F, quote=F)
  
    system(paste0('sbatch --mem 10G -p brc,shared /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/Model_builder/Model_builder_V2_nested.R --pheno /users/k1806347/brc_scratch/Data/UKBB/Phenotype/PRS_comp_subset/UKBB.',target_pheno_i,'.txt --out /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_5_',type,' --assoc T --outcome_pop_prev ',target_pop_prev_i,' --predictors /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_5_',type,'.txt'))
...
for(i in 1:length(target_gwas)){
  for(type in c('rho_restricted_unweighted','rho_fdr_restricted_unweighted','h2_restricted_unweighted','rho_restricted_rho_weighted','rho_fdr_restricted_rho_weighted','h2_restricted_rho_weighted','rho_restricted_rho_h2_weighted','rho_fdr_restricted_rho_h2_weighted','h2_restricted_rho_h2_weighted')){
    ...
    for(secondary_gwas_i in secondary_gwas[secondary_gwas != target_gwas_i]){
      ...
      # Add in remainder of weighted prs for target phenotype
      tmp<-data.frame(predictors=paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/prs_nonhm3/',secondary_gwas_i,'/',target_gwas_i,'/',gsub('restricted_.*','restricted_remainder',type),'/prs.profiles'),
                      group=type)
      
      pred_file<-rbind(pred_file, tmp)
    }
  
    write.table(pred_file, paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_6_',type,'.txt'), col.names=T, row.names=F, quote=F)
  
    system(paste0('sbatch --mem 10G -p brc,shared /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/Model_builder/Model_builder_V2_nested.R --pheno /users/k1806347/brc_scratch/Data/UKBB/Phenotype/PRS_comp_subset/UKBB.',target_pheno_i,'.txt --out /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_6_',type,' --assoc T --outcome_pop_prev ',target_pop_prev_i,' --predictors /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_6_',type,'.txt'))
...
for(i in 1:length(target_gwas)){
  for(type in c('rho_restricted_unweighted','rho_fdr_restricted_unweighted','h2_restricted_unweighted','rho_restricted_rho_weighted','rho_fdr_restricted_rho_weighted','h2_restricted_rho_weighted','rho_restricted_rho_h2_weighted','rho_fdr_restricted_rho_h2_weighted','h2_restricted_rho_h2_weighted')){
    ...
    write.table(pred_file, paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_7_',type,'.txt'), col.names=T, row.names=F, quote=F)
  
    system(paste0('sbatch --mem 10G -p brc,shared /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/Model_builder/Model_builder_V2_nested.R --pheno /users/k1806347/brc_scratch/Data/UKBB/Phenotype/PRS_comp_subset/UKBB.',target_pheno_i,'.txt --out /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_7_',type,' --assoc T --outcome_pop_prev ',target_pop_prev_i,' --predictors /users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_7_',type,'.txt'))
...
lava_res<-fread('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/lava_descript_nonhm3.csv')

lava_res$label<-paste0(lava_res$n_sig_fdr_loci_pos, '/', lava_res$n_sig_fdr_loci_neg)

lava_res$prop_conc<-lava_res$n_sig_fdr_loci_pos/lava_res$n_sig_fdr_loci
lava_res$prop_conc[which(lava_res$prop_conc < 0.5)]<-1-lava_res$prop_conc[which(lava_res$prop_conc < 0.5)]
...
png(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/lava/results/hsq_rg_local_nonhm3.png'), units='px', res=300, width=1900, height=2500)
  plot_grid(plotlist=list(hsq_plot, rg_plot, local_rg_plot), rel_widths=c(0.9,1.2,1.2), nrow=1)
dev.off()
...
for(i in 1:length(target_gwas)){
  target_pheno_i<-target_pheno[i]
  target_gwas_i<-target_gwas[i]
  tmp_assoc<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_',test,'.assoc.txt'))
  ...
  # Some predictors got removed due to containing many infinite values.
  # Merge the predictor file with the assoc results
  tmp_pred<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_',test,'.txt'))
  tmp_pred$ID<-paste0('PredFile',1:nrow(tmp_pred))
  tmp_pred$GWAS<-gsub('/','',gsub(paste0(target_gwas_i,'.*'),'',gsub('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/prs_nonhm3/','',tmp_pred$predictors)))
  tmp_assoc$ID<-gsub('\\..*','',gsub('.*PredFile','PredFile',tmp_assoc$Predictor))
  tmp_assoc<-merge(tmp_assoc, tmp_pred[,c('ID','GWAS'),with=F], by='ID')
  tmp_assoc$ID<-NULL
  tmp_assoc$GWAS<-factor(tmp_assoc$GWAS, levels=unique(tmp_assoc$GWAS))
  ...
  png(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_',test,'.png'), units='px', res=300, width=4500, height=2000)
  print(assoc_plot)
  dev.off()

  assoc_plot_brief<-ggplot(tmp_assoc[tmp_assoc$Filter == 'hsq-p < 0.05',], aes(x=GWAS, y=abs(BETA), fill=Weighting)) +
    geom_bar(stat="identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=abs(BETA)-SE, ymax=abs(BETA)+SE), width=.2, position=position_dodge(.9)) +
    labs(title=paste0('Target: ',target_pheno_i), y="Absolute-R (SE)", x='') +
    theme_half_open() +
  ...
  assoc_plot_brief_unres<-ggplot(tmp_assoc_both[tmp_assoc_both$Filter == 'hsq-p < 0.05' | tmp_assoc_both$Filter == 'Standard',], aes(x=GWAS, y=abs(BETA), fill=Weighting)) +
    geom_bar(stat="identity", position=position_dodge()) +
    geom_errorbar(aes(ymin=abs(BETA)-SE, ymax=abs(BETA)+SE), width=.2, position=position_dodge(.9)) +
    labs(title=paste0('Target: ',target_pheno_i), y="Absolute-R (SE)", x='', fill='Type') +
    theme_half_open() +
  ...
  tmp_eval<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_',test,'.pred_eval.txt'))
  tmp_comp<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_',test,'.pred_comp.txt'))
...
png(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/test_',test,'.png'), units='px', res=300, width=2500, height=2500)
  print(plot_grid(plotlist=eval_plot_list, ncol=1))
dev.off()

png(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/test_',test,'_assoc.png'), units='px', res=300, width=2200, height=2500)
  print(plot_grid(plotlist=assoc_plot_brief_list, ncol=1))
dev.off()

png(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/test_',test,'_assoc_with_unres.png'), units='px', res=300, width=2200, height=2500)
  print(plot_grid(plotlist=assoc_plot_brief_unres_list, ncol=1))
dev.off()

png(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/test_',test,'_lava.png'), units='px', res=300, width=1500, height=2000)
  print(plot_grid(plotlist=lava_plot_list, ncol=1))
dev.off()
...
for(i in 1:length(target_gwas)){
  ...
  for(type in c('rho_restricted_unweighted','rho_fdr_restricted_unweighted','h2_restricted_unweighted','rho_restricted_rho_weighted','rho_fdr_restricted_rho_weighted','h2_restricted_rho_weighted','rho_restricted_rho_h2_weighted','rho_fdr_restricted_rho_h2_weighted','h2_restricted_rho_h2_weighted')){
    target_pheno_i<-target_pheno[i]
    tmp_eval<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_',test,'_',type,'.pred_eval.txt'))
    tmp_comp<-fread(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_',test,'_',type,'.pred_comp.txt'))
  ...
  png(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/',target_pheno_i,'/test_',test,'.png'), units='px', res=300, width=5000, height=3000)
  print(plot_grid(plotlist=eval_plot_list, ncol=3))
  dev.off()
...
png(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/test_',test,'.png'), units='px', res=300, width=4750, height=3000)
print(plot_grid(plotlist=eval_plot_all_list, ncol=3))
dev.off()

###
# Combine rho_restricted results and make simplified plot
# Note. When restricting the hm3 SNPs. we used h2 restricted loci, but when not restricting to hm3 SNP, using rho p<0.05 loci improves prediction.
###
tmp_eval_all_tab<-do.call(rbind, tmp_eval_all)
tmp_comp_all_tab<-do.call(rbind, tmp_comp_all)
...
png(paste0('/users/k1806347/brc_scratch/Analyses/local_rg_pgs/assoc_nonhm3/test_',test,'_brief.png'), units='px', res=300, width=3750, height=1300)
print(plot_grid(plotlist=eval_plot_brief_all_list, ncol=3))
dev.off()
...
LAVA analysis highlighted that the magnitude and direction of genetic correlation between two traits can vary across regions of the genome.

There were more loci with significant h2 when unrestricted to hapmap3 variants. There were fewer loci with FDR significant rG when unrestricted to hapmap3 variants, partly due to larger number of tests (due to more significant h2 loci). Perhaps controlling for all loci is more appropriate than only loci with significant h2 in both traits.

PGS analysis showed that reweighting secondary trait genetic effects according to the local genetic correlation with the target trait often increased the variance explained by the PGS. The effect of rescaling effects according to local heritability difference was less pronounced. The effect of reweighting variants by local genetic correlation was greater when the ratio of shared loci with concordant:discordant effects approached 0.5. There was little difference when restricting loci to those with nominally signficant or FDR significant local genetic correlation.

When modelling target and secondary trait PGS, we replicate the finding that including secondary PGS increases the target trait variance explained. However, there was very little improvement in the predictive utility of models including PGS reweighted according to local genetic correlation. For depression, inclusion of the additional PGS appeared to lead to overfitting, with a small decrease in prediction accuracy. For BMI, relative improvements of ~2.8% were seen, though this appeared to be driven by restricting the PGS to loci with nominal evidence of heritability in both outcomes, with no further gain provided by reweighting by genetic correlation. For intelligence, there was a small but significant relative improvement of 0.3%, achieved only reweighting variants by local genetic correlation.

These findings collectively highlight the potential benefit of reweighting PGS according to evidence of local heritability aross traits, and the direction and magnitude of genetic correlation between traits. Though the relative gains in prediction accuracy are dependent on the secondary traits included and their relationship to the target phenotype.
