...
cat(
'#################################################################
# Scaled_polygenic_scorer_DBSLMM.R V1.0
...
scores$SCORE_DBSLMM<-scores$SCORE_DBSLMM-ref_scale$Mean
scores$SCORE_DBSLMM<-scores$SCORE_DBSLMM/ref_scale$SD
scores$SCORE_DBSLMM<-round(scores$SCORE_DBSLMM,3)
...
fwrite(scores, paste0(opt$output,'.DBSLMM_profiles'), sep=' ')

sink(file = paste(opt$output,'.log',sep=''), append = T)
cat('Saved polygenic scores to: ',opt$output,'.DBSLMM_profiles.\n',sep='')
sink()
