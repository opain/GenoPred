...
cat(
'#################################################################
# Scaled_polygenic_scorer_SBLUP.R V1.0
...
if(is.na(opt$target_keep)){
	for(i in 1:22){
		system(paste0(opt$plink, ' --bfile ',opt$target_plink_chr,i,' --read-freq ',opt$ref_freq_chr,i,'.frq --score ',opt$ref_score,' 1 2 4 sum --out ',opt$output_dir,'profiles.chr',i,' --memory ',floor(opt$memory*0.9)))
	}
} else {
	for(i in 1:22){
		system(paste0(opt$plink, ' --bfile ',opt$target_plink_chr,i,' --read-freq ',opt$ref_freq_chr,i,'.frq --keep ',opt$target_keep,' --score ',opt$ref_score,' 1 2 4 sum --out ',opt$output_dir,'profiles.chr',i,' --memory ',floor(opt$memory*0.9)))
	}
}
...
names(scores)[grepl('SCORE_temp',names(scores))]<-'SCORE_SBLUP'
...
scores$SCORE_SBLUP<-scores$SCORE_SBLUP-ref_scale$Mean
scores$SCORE_SBLUP<-scores$SCORE_SBLUP/ref_scale$SD
scores$SCORE_SBLUP<-round(scores$SCORE_SBLUP,3)
...
fwrite(scores, paste0(opt$output,'.SBLUP_profiles'), sep=' ')

sink(file = paste(opt$output,'.log',sep=''), append = T)
cat('Saved polygenic scores to: ',opt$output,'.SBLUP_profiles.\n',sep='')
sink()
