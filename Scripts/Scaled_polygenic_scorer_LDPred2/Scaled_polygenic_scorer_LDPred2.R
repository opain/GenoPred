...
cat(
'#################################################################
# Scaled_polygenic_scorer_LDPred2.R V1.0
...
foreach(param_i=param, .combine=c)%dopar%{
  if(is.na(opt$target_keep)){
		for(i in CHROMS){
			system(paste0(opt$plink, ' --bfile ',opt$target_plink_chr,i,' --read-freq ',opt$ref_freq_chr,i,'.frq --score ',opt$ref_score,'.',param_i,'.SCORE 1 2 4 sum --out ',opt$output_dir,'profiles.',param_i,'.chr',i,' --memory ',floor((opt$memory*0.9)/opt$n_cores)))
		}
	} else {
		for(i in CHROMS){
			system(paste0(opt$plink, ' --bfile ',opt$target_plink_chr,i,' --read-freq ',opt$ref_freq_chr,i,'.frq --keep ',opt$target_keep,' --score ',opt$ref_score,'.',param_i,'.SCORE 1 2 4 sum --out ',opt$output_dir,'profiles.',param_i,'.chr',i,' --memory ',floor((opt$memory*0.9)/opt$n_cores)))
		}
	}
}
...
ref_scale<-fread(opt$ref_scale)
ref_scale$Mean<-as.numeric(ref_scale$Mean)
ref_scale$SD<-as.numeric(ref_scale$SD)
