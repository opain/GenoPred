...
cat(
'#################################################################
# Scaled_polygenic_scorer_PRScs.R V1.0
...
phi_param<-ref_scale$Param
phi_param<-gsub('SCORE_phi','',phi_param)
...
for(phi_i in phi_param){
	if(is.na(opt$target_keep)){
		for(i in 1:22){
			system(paste0(opt$plink, ' --bfile ',opt$target_plink_chr,i,' --read-freq ',opt$ref_freq_chr,i,'.frq --score ',opt$ref_score,'_pst_eff_a1_b0.5_phi',phi_i,'_chr',i,'.txt 2 4 6 sum --out ',opt$output_dir,'profiles.',phi_i,'.chr',i,' --memory ',floor(opt$memory*0.9)))
		}
	} else {
		for(i in 1:22){
			system(paste0(opt$plink, ' --bfile ',opt$target_plink_chr,i,' --read-freq ',opt$ref_freq_chr,i,'.frq --keep ',opt$target_keep,' --score ',opt$ref_score,'_pst_eff_a1_b0.5_phi',phi_i,'_chr',i,'.txt 2 4 6 sum --out ',opt$output_dir,'profiles.',phi_i,'.chr',i,' --memory ',floor(opt$memory*0.9)))
		}
	}
}
...
for(phi_i in phi_param){
	SCORE_temp<-0
	for(i in 1:22){
			profile<-fread(paste0(opt$output_dir,'profiles.',phi_i,'.chr',i,'.profile'))
...
fwrite(scores, paste0(opt$output,'.PRScs_profiles'), sep=' ')

sink(file = paste(opt$output,'.log',sep=''), append = T)
cat('Saved polygenic scores to: ',opt$output,'.PRScs_profiles.\n',sep='')
sink()
