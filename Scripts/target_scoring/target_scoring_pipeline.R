...
if(opt$continuous){
  ...
  for(i in 1:nrow(score_files)){
    print(i)
    models[[paste0('score_file_', i)]]<-readRDS(paste0(outdir, '/reference/pgs_score_files/', score_files$method[i],'/', score_files$name[i],'/ref-',score_files$name[i],'-', opt$population, '.model.rds'))
