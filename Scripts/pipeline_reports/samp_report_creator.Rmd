...
# Read in raw psudoval PGS across all indivduals
ref_pgs <- read_pgs(config = params$config, ref = T, pop = 'TRANS', pseudo_only = T, correction = 'raw')$reference$TRANS

# Merge all PGS into single data.frame
for(gwas_i in names(ref_pgs)){
  for(method_i in names(ref_pgs[[gwas_i]])){
    names(ref_pgs[[gwas_i]][[method_i]]$raw)[3] <- paste(gwas_i,method_i, sep=':')
    ref_pgs[[gwas_i]][[method_i]] <- ref_pgs[[gwas_i]][[method_i]]$raw
  }
}
...
cat0("**Note.** Plot shows correlation between pseudovalidated PGS.\n\n")
cat0("***\n\n")
...
cat0("### PGS coverage \n\n")
cat0("This section summarises the polygenic score files, including both GWAS-derived and external scores. It reports the number of SNPs per score, the proportion present in the target sample, and the relative R² accounting for missing variants. These metrics provide a check on score completeness, transferability, and the expected predictive utility in the target data.\n\n")
cat0("***\n\n")
...
# Read in prs
corrections <- 'raw'
if(!is.na(pgs_scaling)){
  corrections <- c(corrections, pgs_scaling)
}
pgs <- read_pgs(config = params$config, name = params$name, pseudo_only=T, correction = corrections)[[1]]
...
for(gwas in names(pgs[[1]])){
  for(pgs_method in names(pgs[[1]][[gwas]])){
    for(pop in names(pgs)){
      for(correction in names(pgs[[pop]][[gwas]][[pgs_method]])){
        tmp <- pgs[[pop]][[gwas]][[pgs_method]][[correction]]
...
for(pgs_method_i in unique(pgs$pgs_method)){
  for(correction_i in unique(pgs$correction)){
    pgs_i <- pgs[pgs$pgs_method == pgs_method_i & pgs$correction == correction_i,]
    tmp_plot <- 
      ggplot(pgs_i, aes(x=score, fill = pop)) + 
    ...
    png(paste0(tmp_folder,'/plot_', pgs_method_i,'.', correction_i,'.png'), height = 150 + (300*length(unique(pgs_i$gwas))), width = plot_width, res = 170)
      print(tmp_plot)
    dev.off()
  }
}
...
for(correction_i in unique(pgs$correction)){
  # Create a new tab for each correction
  cat0("#### ", correction_i, "\n")
  for(pgs_method_i in unique(pgs$pgs_method)){
    ...
    # Show the plot
    cat0(paste0("![Density plot of PGS from ", pgs_method_i,"](", tmp_folder,'/plot_',pgs_method_i,'.', correction_i,".png)"))
