...
for(gwas in names(pgs[[1]])){
  for(pgs_method in names(pgs[[1]][[gwas]])){
    pseudo_param <- find_pseudo(config = params$config, gwas = gwas, pgs_method = pgs_method, target_pop = 'TRANS')
...
for(i in names(ancestry_log$keep_files)){
  pgs_pseudo$assigned_pop[paste0(pgs_pseudo$FID, '.', pgs_pseudo$IID) %in% paste0(ancestry_log$keep_files[[i]]$V1, '.',  ancestry_log$keep_files[[i]]$V2)] <- i
}
pgs_pseudo$assigned_pop[is.na(pgs_pseudo$assigned_pop)]<-'Unassigned'
...
for(pgs_method_i in unique(pgs_pseudo$pgs_method)){
  ...
  pgs_dist_plots[[pgs_method_i]] <- 
    ggplot(pgs_pseudo_i, aes(x=score, fill=assigned_pop)) + 
