...
names(sumstat_qc) <- c('Name', 'Label', 'Population', 'NSNP Original', 'NSNP Final','Strand check')
...
# Remove gwas with invalid pseudoval results
pseudoval_clean <- pseudoval
pseudoval_clean$pseudoval_r2obs[pseudoval$pseudoval_r2obs < 0 | pseudoval$pseudoval_r2obs > 1] <- NA
pseudoval_clean$pseudoval_auc[pseudoval$pseudoval_auc < 0 | pseudoval$pseudoval_auc > 1] <- NA
pseudoval_clean <- pseudoval[apply(pseudoval[,-1], 1, function(x) !all(is.na(x))),]

# Calculate absolute risk
abs_res <- NULL
n_quant <- 1000
for(gwas_i in unique(pseudoval_clean$gwas)){
  for(pgs_method_i in unique(pgs_dat$pgs_method[pgs_dat$gwas == gwas_i])){
    ...
    if (!is.na(gwas_list$mean[gwas_list$name == gwas_i]) & !is.na(gwas_list$mean[gwas_list$name == gwas_i])) {
      quant <-
    ...
    if (!is.na(gwas_list$prevalence[gwas_list$name == gwas_i]) & !is.na(gwas_list$sampling[gwas_list$name == gwas_i])) {
      risk_quantiles <-
        ccprobs.f(
...
for(gwas_i in unique(pgs_dat$gwas)){
  for(pgs_method_i in unique(pgs_dat$pgs_method[pgs_dat$gwas == gwas_i])){
    ...
    if(pgs_method_i != 'external' & gwas_i %in% abs_res$gwas & gwas_i %in% pseudoval_clean$gwas){
      ...
      abs_plot <- ggplot() +
        annotate(
          ...
          x = 0.5,
          y = 0.5,
          label = "Automatic conversion to absolute scale is unavailable.\nEither due to use of external PGS, population distribution,\n unspecified in gwas_list, or invalid estimated PGS accuracy metric.",
          size = 4,
          hjust = 0.5,
          vjust = 0.5
        ) +
...
all_dat<-merge(pgs_dat, abs_res, by=c('gwas','pgs_method'), all.x = T)
all_dat<-merge(all_dat, score_info, by='gwas', all = T)
...
for(pgs_method_i in pgs_methods_list){
  ...
  for(gwas_i in unique(pgs_dat$gwas[pgs_dat$pgs_method == pgs_method_i])){
    ...
    cat0('- Your PGS Z-score for ', all_dat$label[all_dat$gwas == gwas_i & all_dat$pgs_method == pgs_method_i], ' is ', pgs_dat_i$value,' which is higher than ', paste0(round(100*pnorm(pgs_dat_i$value),1),'%'),' of other ', top_pop,' individuals.\n\n')
    
    if(gwas_i %in% abs_res$gwas){
      if(is.na(gwas_list$prevalence[gwas_list$name == gwas_i])){
