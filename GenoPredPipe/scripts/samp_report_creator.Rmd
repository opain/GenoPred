...
output<-target_list$output[target_list$name == params$name]

harmoniser_log<-readLines(paste0(output,'/',params$name,'/',params$name,'.1KGphase3.hm3.chr.hm3_harmoniser.log'))

fam<-fread(paste0(output,'/',params$name,'/',params$name,'.1KGphase3.hm3.chr22.fam'))
...
model_pred<-fread(paste0(output,'/',params$name,'/ancestry/ancestry_all/',params$name,'.Ancestry.model_pred'))
...
knitr::include_graphics(normalizePath(paste0(output,'/',params$name,'/ancestry/ancestry_all/', params$name,'.Ancestry.PCs_plot_super_pop.png')))
```
...
for(population in unique(model_pred_hard$Population)){
  ...
  model_pred<-fread(paste0(output,'/',params$name,'/ancestry/ancestry_',population,'/',params$name,'.Ancestry.model_pred'))
  probs_melt<-melt(model_pred[,-1:-2])
...
for(gwas_i in gwas_list$name){
  prs_i<-fread(paste0(output,'/',params$name,'/prs/',population,'/dbslmm/',gwas_i,'/',params$name,'.',gwas_i,'.',population,'.profiles'))
