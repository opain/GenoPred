...
harmoniser_log<-readLines(paste0('resources/data/target/',params$name,'/',params$name,'.1KGphase3.hm3.chr.hm3_harmoniser.log'))

fam<-fread(paste0('resources/data/target/',params$name,'/',params$name,'.1KGphase3.hm3.chr22.fam'))

ref_bim<-fread('resources/data/1kg/1KGPhase3.w_hm3.GW.bim')
...
model_pred<-fread(paste0('resources/data/target/',params$name,'/ancestry/',params$name,'.Ancestry.model_pred'))
...
knitr::include_graphics(normalizePath(paste0('resources/data/target/',params$name,'/ancestry/', params$name,'.Ancestry.PCs_plot_super_pop.png')))
```
...
for(population in unique(model_pred_hard$Population)){
  ...
  model_pred<-fread(paste0('resources/data/target/',params$name,'/ancestry_',population,'/',params$name,'.Ancestry.model_pred'))
  probs_melt<-melt(model_pred[,-1:-2])
  ...
  cat("Of the ",nrow(model_pred_hard_pop)," individuals assigned to the ",population," super population, the ancestry model classified ",n_hard_call," individuals to a specific ",super_pop_codes$pop[super_pop_codes$code == population]," population with a probability >50%. The number of individuals assigned to each ",super_pop_codes$pop[super_pop_codes$code == population]," population were: \n", sep='')
...
for(gwas_i in gwas_list$name){
  prs_i<-fread(paste0('resources/data/target/',params$name,'/prs/',population,'/dbslmm/',gwas_i,'/',params$name,'.',gwas_i,'.',population,'.DBSLMM_profiles'))
...
```{R, eval=T, echo=F, message=F, warning=F, fig.cap="Distribution of DBSLMM PRS", fig.dim = c(8, 1.5*nrow(gwas_list))}
...
dbslmm_prs_all_2<-dbslmm_prs_all
dbslmm_prs_all_2$label<-'All'
dbslmm_prs_all_2$pop<-'All'
dbslmm_prs_all<-rbind(dbslmm_prs_all,dbslmm_prs_all_2)
