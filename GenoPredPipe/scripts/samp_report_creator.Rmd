...
params:
  name: "opensnp"
  config: "misc/opensnp/config.yaml"
output:
  html_document:
    toc: true
    theme: paper
...
cat0("- ", sum(target_classifications$N[target_classifications$Population != 'Unassigned']), " out of ", nrow(target_fam), " target individuals in the sample, were assigned to a reference population with a probability >95%.\n")
...
for(pgs_method_i in unique(pgs_pseudo$pgs_method)){
  ...
  png(paste0(tmp_folder,'/plot_', pgs_method_i,'.png'), height = 200 + (800*length(unique(pgs_pseudo_i$gwas))), width = 3000, res = 300)
