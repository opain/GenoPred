...
if(imp_incl){
  ...
  id_tmp <- target_fam[paste0(target_fam$V1,'.',target_fam$V2) == params$id,] 
...
if(nrow(keep_all) == 0){
  ...
  cat0("- The indivdual best matches the ", pop_1kg$label[pop_1kg$pop == top_pop], " reference population, but did not reach the ", ancestry_prob_thresh*100, "% probability threshold.\n\n")
  pgs_incl <- F
} else {
  cat0("- The target individual matches the ", pop_1kg$label[pop_1kg$pop == keep_all$pop], " reference population with a probability >", ancestry_prob_thresh*100, "%.\n\n")
}
...
dat[dat == 0] <-NA
dat <- merge(dat, pop_1kg, by.x = 'variable', by.y = 'pop')
dat$label <- gsub(' ', '\n', dat$label)
dat$annot <- paste0(dat$value*100, '%')
y_max <- ifelse(max(dat$value, na.rm = T) > 0.95, 105, 100)

png(paste0(tmp_folder,'/plot_ancestry.png'), height = 450, width = 600, res = 120)
...
# Show the plot
cat0(paste0("![Ancestry inference: Population probabilities.](", tmp_folder,"/plot_ancestry.png){width=500px}\n\n"))
...
for(pgs_method_i in unique(pgs_dat$pgs_method[pgs_dat$gwas == gwas_i])){
  ...
  for(gwas_i in unique(pgs_dat$gwas)){
    ...
    cat0('- Your PRS Z-score for ', gwas_list$label[gwas_list$name == gwas_i], ' is ', pgs_dat_i$value,' which is higher than ', paste0(round(100*pnorm(pgs_dat_i$value),1),'%'),' of other ', pop_1kg$label[pop_1kg$pop == top_pop],' individuals.\n')
    
    if(is.na(gwas_list$prevalence[gwas_list$name == gwas_i])){
      
      cat0('- Given the PRS explains ', round(100*pseudoval$pseudoval_r2obs[pseudoval$gwas == gwas_i],1),'% of the variance in ', gwas_list$label[gwas_list$name == gwas_i],',\n and the mean and SD of ', gwas_list$label[gwas_list$name == gwas_i],' in the general population is ',gwas_list$mean[gwas_list$name == gwas_i],' and ',gwas_list$sd[gwas_list$name == gwas_i],' respectively,\n on average people with your PRS have a ', gwas_list$label[gwas_list$name == gwas_i],' of ', round(abs_res_i$mean,2), " (95% CI = ",round(lowCI,2),' - ',round(highCI,2),").\n\n")

    } else {
        
      cat0('- Given the PRS has an AUC of ', round(pseudoval$pseudoval_auc[pseudoval$gwas == gwas_i],3),', and the prevelance of ', gwas_list$label[gwas_list$name == gwas_i],' in the general population is ',gwas_list$prevalence[gwas_list$name == gwas_i],',\n on average ',round(100*abs_res_i$abs_prob,1),'% of people with your PRS have ', gwas_list$label[gwas_list$name == gwas_i],'.\n\n')
