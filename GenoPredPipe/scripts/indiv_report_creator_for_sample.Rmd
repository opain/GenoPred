...
```{r, eval=max(model_pred[1,-1:-2]) <= 0.95, results='asis', echo=FALSE}

cat('The individual could not be assigned to a reference population.\n\n')

```

```{R, eval=max(model_pred[1,-1:-2]) > 0.95, echo=F, message=F, warning=F}
...
for(gwas_i in gwas_list$name){
  prs_i<-fread(paste0(params$output,'/',params$name,'/prs/',pred_pop,'/dbslmm/',gwas_i,'/',params$name,'.',gwas_i,'.',pred_pop,'.profiles'))
...
pseudoval<-NULL
for(gwas_i in gwas_list$name){
  pseudoval_log<-read.table(paste0('resources/data/ref/prs_pseudoval/',gwas_i,'/lassosum_pseudo_',gwas_i,'.log'), sep='&', header=F)$V1
...
```{r, eval=max(model_pred[1,-1:-2]) > 0.95, echo=F}
...
```{r, eval=max(model_pred[1,-1:-2]) > 0.95, results='asis', echo=FALSE, fig.dim=c(10,9), dpi = 70, optipng = '-o7'}
