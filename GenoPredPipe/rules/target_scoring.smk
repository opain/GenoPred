...
rule target_prs:
  ...
  output:
    touch("{outdir}/resources/data/target_checks/{name}/target_prs_{method}_{population}_{gwas}.done")
  conda:
    "../envs/GenoPredPipe.yaml"
  params:
...
rule run_target_prs_all_gwas:
  input:
    config["gwas_list"],
    lambda w: expand("{outdir}/resources/data/target_checks/{name}/target_prs_{method}_{population}_{gwas}.done", name=w.name, gwas= score_list_df['name'] if w.method == 'external' else (gwas_list_df['name'] if w.method in ['dbslmm','lassosum','megaprs'] else gwas_list_df['name']), population=w.population, method=w.method, outdir=outdir)
  output:
    touch("{outdir}/resources/data/target_checks/{name}/run_target_prs_{method}_all_gwas_{population}.done")

rule run_target_prs_all_pop:
  input: 
    lambda w: expand("{outdir}/resources/data/target_checks/{name}/run_target_prs_{method}_all_gwas_{population}.done", name=w.name, population=ancestry_munge("{}".format(w.name)), method=w.method, outdir=outdir)
  output:
    touch("{outdir}/resources/data/target_checks/{name}/run_target_prs_{method}_all_pop.done")

rule run_target_prs_all_method:
  input: 
    lambda w: expand("{outdir}/resources/data/target_checks/{name}/run_target_prs_{method}_all_pop.done", method=pgs_methods, name=w.name, outdir=outdir)
  output:
    touch("{outdir}/resources/data/target_checks/{name}/run_target_prs_all_method.done")

rule run_target_prs_all_name:
  input: 
    expand("{outdir}/resources/data/target_checks/{name}/run_target_prs_all_method.done", name=target_list_df['name'], outdir=outdir)
  output:
    touch("{outdir}/resources/data/target_checks/run_target_prs_all_name.done")
