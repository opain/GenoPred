...
# Create new dnorm function that allows specification of height
# This is used for plotting normal distributions
dnorm_new<-function(x, mean=0, sd=1, log=F, height=1){
  out<-dnorm(x=x,mean=mean,sd=sd,log=log)
  out*height
}

# Create dnorm function that is the sum of two normal distributions with different parameters
# This is used for plotting normal distributions
...
# Create a function estimating the proportion of cases expected within quantiles of a normally distributed predictor (i.e. a PGS)
# The function requires the effect size of the predictor on the outcome in units of Cohen's D, the prevelance of the outcome in the general population, and the number of quantiles that the predictor should be split into.
...
# This function returns the quantile corresponding to a normally distributed Z score (mean = 0, and sd = 1)
...
# This function estimates the mean and standard deviation of an outcome within quantiles of a normally distributed predictor
# The variance examplained by the predictor, and the mean and SD of the outcome in the general population is required.
