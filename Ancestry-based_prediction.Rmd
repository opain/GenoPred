...
groups<-data.frame( predictor=c('/users/k1806347/brc_scratch/Data/UKBB/Projected_PCs/EUR/UKBB.w_hm3.EUR.eigenvec','/users/k1806347/brc_scratch/Data/UKBB/Projected_PCs/AllAncestry/UKBB.w_hm3.AllAncestry.EUR.eigenvec'),
...
gwas=$(echo DEPR07 COLL01 BODY03 HEIG03)
...
<details><summary>Evaluate PCs in combination with polygenic scores</summary>
```{bash, echo=T, eval=F}
# Make required directories
for pheno_i in $(echo Depression Intelligence BMI Height);do
mkdir -p /users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/${pheno_i}/Association_withPRSs_and_PCs
done
...
gwas<-c('DEPR07','COLL01','BODY03','HEIG03')
pheno<-c('Depression','Intelligence','BMI','Height')

for(i in 1:4){
	groups<-data.frame( predictor=paste0('/users/k1806347/brc_scratch/Data/UKBB/PolygenicScores/EUR/',gwas[i],'/UKBB.w_hm3.',gwas[i],'.EUR.profiles'),
							  group=paste0(gwas[i],'_PRSs'))

	groups<-rbind(data.frame( predictor=c('/users/k1806347/brc_scratch/Data/UKBB/Projected_PCs/EUR/UKBB.w_hm3.EUR.eigenvec','/users/k1806347/brc_scratch/Data/UKBB/Projected_PCs/AllAncestry/UKBB.w_hm3.AllAncestry.EUR.eigenvec'),
			  group='PCs'),
	groups)

	write.table(groups, paste0('/users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/',pheno[i],'/Association_withPRSs_and_PCs/UKBB_',gwas[i],'_PRSs-ProjPCs.predictor_groups'), col.names=T, row.names=F, quote=F)
}
q()
n
...
qsub -l h_vmem=20G -pe smp 3 /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/Scripts/Model_builder.R \
--pheno /users/k1806347/brc_scratch/Data/UKBB/Phenotype/${pheno_i}/${pheno_file_i} \
--keep /users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/ukb18177_glanville_post_qc_id_list.UpdateIDs.fam \
--out /users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/${pheno_i}/Association_withPRSs_and_PCs/UKBB_${gwas_i}_PRSs-ProjPCs \
--n_core 3 \
--assoc T \
--outcome_pop_prev ${prev_i} \
--predictors /users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/${pheno_i}/Association_withPRSs_and_PCs/UKBB_${gwas_i}_PRSs-ProjPCs.predictor_groups
done
```
</details>
