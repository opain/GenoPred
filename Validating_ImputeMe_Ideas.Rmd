...
### Example with low AUC

<details><summary>Show code</summary>
```{R, eval=F, echo=T}

# Alex gillet has kindly written a function which calculates the number of cases and controls across PRS quantiles based on the prevelance of the outcome and the AUC of the PRS.
...
####
# Predict the probability an individual is a case
####
...
risk_quantiles<-ccprobs.f(PRS_auc=AUC, prev=prev, n_quantile=100)
...
indiv_result$value<-indiv_result$value
  
# Shows probability of being a case or a control
bar_plot<-ggplot(indiv_result, aes(x=variable, y=value*100, fill=variable)) +
  geom_bar(stat="identity") +
  ylim(0,NA) +
  labs(x='', y='Percentage Case/Control', title='Percentage of People Like you with the Outcome') +
...
plot_dat<-(matrix(c(rep('Control',n_control),rep('Case',n_case)),nrow=10, ncol=10))
plot_dat<-data.frame(melt(plot_dat))
plot_dat$image<-'person_symbol.png'

library(ggplot2)
library(ggimage)

# Plot2
person_plot<-ggplot(plot_dat, aes(x=Var1, y=Var2, colour=value)) +
  geom_image(aes(image=image), size=.05) +
  labs(colour='Status', title=paste0(n_case,'% of people like you have the outcome')) +
...
# Plot 3
text_plot<-ggplot(data.frame(x=0,y=0), aes(x=x, y=y)) +
  xlim(0,5) +
  geom_text(label=paste('Outcome: T2D
Prevelance: 0.03
PRS AUC: 0.58

Your PRS: ', PRS_z_score,'
Your PRS percentile: ',as.numeric(row.names(indiv_result_all)),'th
Your absolute risk: ',n_case,'

Out of 100 people like you, ',n_case,' have a diagnosis of T2D'), hjust=0) +
  theme(axis.line=element_blank(),axis.text.x=element_blank(),
        axis.text.y=element_blank(),axis.ticks=element_blank(),
        axis.title.x=element_blank(),
        axis.title.y=element_blank(),
...
png('Images/Validating_ImputeMe_Ideas/Plotting_binary_outcome.png', res=300, unit='px', width=6000, height=1750)
plot_grid(text_plot, bar_plot, person_plot, labels = "AUTO", ncol = 3)
dev.off()

```
</details>

<details><summary>Show individual risk of outcome</summary>

<center>

![Individual risk plot](Images/Validating_ImputeMe_Ideas/Plotting_binary_outcome.png)
...
png('Images/Validating_ImputeMe_Ideas/Plotting_binary_outcome_highAUC.png', res=300, unit='px', width=6000, height=1750)
plot_grid(text_plot, bar_plot, person_plot, labels = "AUTO", ncol = 3)
dev.off()
...
![Individual risk plot](Images/Validating_ImputeMe_Ideas/Plotting_binary_outcome_highAUC.png)
...
```{R, eval=F, echo=T}
# Create function 
PRS_abs_quant2<-function(n=10000, PRS_R2=0.3, Outcome_mean=1, Outcome_sd=1, n_quantile=20, seed=1){
...
# Create a plot showing an individuals BMI prediction against the UKB sample
PRS_z_score<--1.7
Outcome_mean<-27.20312
Outcome_sd<-4.361187
PRS_R2<-0.07
...
lowCI<-qnorm(0.05,indiv_result_all$x_mean,indiv_result_all$x_sd)
highCI<-qnorm(0.95,indiv_result_all$x_mean,indiv_result_all$x_sd)

library(ggplot2)
library(cowplot)

png('Images/Validating_ImputeMe_Ideas/Plotting_continuous_outcome.png', res=300, unit='px', width=1750, height=1250)
ggplot(plot_dat, aes(x=x, y=y, fill=Group)) +
  geom_area(alpha=0.8) +
  annotate("text",  x=Inf, y = Inf, label = paste0('Estimate = ',round(indiv_result_all$x_mean,1)), vjust=1, hjust=1) +
  labs(x='BMI', y='Density', title='Your BMI compared to general population') +
  geom_vline(aes(xintercept=indiv_result_all$x_mean), size=1) +
  geom_vline(aes(xintercept=lowCI), color="blue", linetype="dashed", size=1) +
  geom_vline(aes(xintercept=highCI), color="blue", linetype="dashed", size=1) +
  annotate("text",  x=Inf, y = Inf, label = paste0('95% CI = ',round(lowCI,1),'-',round(highCI,1)), colour = 'blue', vjust=3, hjust=1) +
  theme_minimal(12) +
...
<details><summary>Show individual prediction of outcome</summary>

<center>

![Individual risk plot](Images/Validating_ImputeMe_Ideas/Plotting_continuous_outcome.png)
...
png('Images/Validating_ImputeMe_Ideas/Plotting_continuous_outcome_highR2.png', res=300, unit='px', width=1750, height=1250)
ggplot(plot_dat, aes(x=x, y=y, fill=Group)) +
  geom_area(alpha=0.8) +
  annotate("text",  x=Inf, y = Inf, label = paste0('Estimate = ',round(indiv_result_all$x_mean,1)), vjust=1, hjust=1) +
  labs(x='BMI', y='Density', title='Your BMI compared to general population') +
...
![Individual risk plot](Images/Validating_ImputeMe_Ideas/Plotting_continuous_outcome_highR2.png)
