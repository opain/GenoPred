...
res[,-1]<-round(res[,-1],3)
res<-res[,c('Phenotype','N','vg1','Intercept','prevalence','sampling','pred_AUC_LDSC','pred_AUC_AVENG','pTclump_pt1_AUC','DBSLMM_AUC')]
names(res)<-c('Phenotype','N','LDSC h2','LDSC intercept','Prevalence','Sampling',"Obs. AUC (pT1)", "Obs. AUC (DBSLMM)","Est. AUC (LDSC h2)", "Est. AUC (AVENGEME h2)")
...
for(i in 1:dim(files)[1]){
  ...
  plots_all[[i]]<-ggplot(quant_comp[grepl('Estimated', quant_comp$Type),], aes(x=Quantile, y=p_case, colour=Type)) +
    geom_line() +
...
for(i in 1:dim(files)[1]){
  ...
  plots_all[[i]]<-ggplot(quant_comp[grepl('Estimate', quant_comp$Type),], aes(x=Quantile, y=x_mean, colour=Type)) +
