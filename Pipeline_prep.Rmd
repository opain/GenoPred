...
for(i in 3:3){
  ...
  if(length(files) == nsnp_chunk){
    ...
    system(paste0('qsub -b y -l h_vmem=',round(2.1*nsnp_chunk),'G ',gctb,' --mldm ',Geno_1KG_dir,'/LD_matrix/EUR/shrunk_ld_chr',i,'merge_list --make-shrunk-ldm --out ',Geno_1KG_dir,'/LD_matrix/EUR/1KGPhase3.w_hm3.EUR.chr',i))
  } else {
...
for chr in $(seq 3 3);do 
qsub -b y -q LowMemShortterm.q@nodec01,LowMemShortterm.q@nodec10,LowMemShortterm.q@nodec05,LowMemShortterm.q@nodec16,LowMemShortterm.q@nodec06 -l h_vmem=30G ${gctb} --ldm ${Geno_1KG_dir}/LD_matrix/EUR/1KGPhase3.w_hm3.EUR.chr${chr}.ldm.shrunk --chisq 0 --make-sparse-ldm --out ${Geno_1KG_dir}/LD_matrix/EUR/1KGPhase3.w_hm3.EUR.chr${chr}
done
...
ls ${Geno_1KG_dir}/LD_matrix/EUR/1KGPhase3.w_hm3.EUR.chr*.ldm.sparse.bin | sed "s/.bin//" > ${Geno_1KG_dir}/LD_matrix/EUR/1KGPhase3.w_hm3.EUR.sparse.ldm.list
```
...
opt$ld_matrix<-'/users/k1806347/brc_scratch/Data/1KG/Phase3/LD_matrix/EUR/1KGPhase3.w_hm3.EUR.chr'
opt$memory<-8000
opt$n_cores<-1
opt$output<-'/users/k1806347/brc_scratch/Data/1KG/Phase3/Score_files_for_poylygenic_SBayesR/HEIG03/1KGPhase3.w_hm3.HEIG03'
opt$ref_pop_scale<-'/users/k1806347/brc_scratch/Data/1KG/Phase3/super_pop_keep.list'
...
opt$ld_matrix<-'/users/k1806347/brc_scratch/Data/1KG/Phase3/LD_matrix/GCTB_shared/ukbEURu_hm3_shrunk_sparse/ukbEURu_hm3_chr3_v3_50k'
...
/users/k1806347/brc_scratch/Software/gctb_2.0_Linux/gctb --sbayes R --ldm /users/k1806347/brc_scratch/Data/1KG/Phase3/LD_matrix/GCTB_shared/ukbEURu_hm3_shrunk_sparse/ukbEURu_hm3_chr3_v3_50k.ldm.sparse --pi 0.95,0.02,0.02,0.01 --gamma 0.0,0.001,0.1,1 --gwas-summary /users/k1806347/brc_scratch/Data/1KG/Phase3/Score_files_for_poylygenic_SBayesR/HEIG03/GWAS_sumstats_COJO.txt --chain-length 10000 --exclude-mhc --burn-in 2000 --out-freq 100 --out /users/k1806347/brc_scratch/Data/1KG/Phase3/Score_files_for_poylygenic_SBayesR/HEIG03/GWAS_sumstats_SBayesR.chr3 > GCTB_LD_mat.out

# The hsq estimates was blowing up quickly because some SNPs contained na values for the se due to Z scores of exactly 0.
# I remove those SNPs and the hsq still blows up after 3000 iterations, even when using the gctb ldmatrix. 
# If I truncate the sample size distribution (i.e. remove SNPs with N >= 0.9*max(N)), the analysis finishes when using the gctb ld matrix, but not the 1KG reference. It also fails with my UKBB reference but after more iterations.

# Create cojo format GWAS sumstats from the original GWAS sumstats.
GWAS<-fread(cmd=paste0('zcat /scratch/groups/ukbiobank/sumstats/cleaned/HEIG03.gz'))
