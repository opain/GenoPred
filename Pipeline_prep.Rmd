...
mkdir -p ${Geno_1KG_dir}/PRSs_LD_matrix/LD_Blocks
mkdir -p ${Geno_1KG_dir}/PRSs_LD_matrix/EUR
...
# Read in the freq files to create the snopinfo file
frq_all<-NULL
for(i in 1:22){
...
write.table(frq_bim, paste0(Geno_1KG_dir,'/PRSs_LD_matrix/EUR/snpinfo_1kg_hm3'), col.names=T, row.names=F, quote=F)
...
blk_chr<-bed$chr
write.table(blk_chr,paste0(Geno_1KG_dir,'/PRSs_LD_matrix/LD_Blocks/blk_chr'), col.names=F, row.names=F, quote=F)
...
for(i in 1:dim(bed)[1]){
  ...
  write.table(frq_bim_i$SNP,paste0(Geno_1KG_dir,'/PRSs_LD_matrix/LD_Blocks/Block_',i,'.snplist'), col.names=F, row.names=F, quote=F)
}

write.table(blk_size,paste0(Geno_1KG_dir,'/PRSs_LD_matrix/LD_Blocks/blk_size'), col.names=F, row.names=F, quote=F)
...
sbatch -p brc,shared ${plink1_9} \
    ...
    --extract ${Geno_1KG_dir}/PRSs_LD_matrix/LD_Blocks/Block_${block}.snplist \
    --r square \
    --keep ${Geno_1KG_dir}/keep_files/EUR_samples.keep \
    --out ${Geno_1KG_dir}/PRSs_LD_matrix/LD_Blocks/1KGPhase3.w_hm3.EUR.Block_${block}
done

for block in $(seq 1 1703);do 
if [ ! -f ${Geno_1KG_dir}/PRSs_LD_matrix/LD_Blocks/1KGPhase3.w_hm3.EUR.Block_${block}.ld ]; then
echo $block
fi
done
...
for(i in c(1:1186,1189:1703)){
  LD_mat<-as.matrix(fread(paste0(Geno_1KG_dir,'/PRSs_LD_matrix/LD_Blocks/1KGPhase3.w_hm3.EUR.Block_',i,'.ld')))
...
sbatch -p brc,shared -n 6 --mem 6G /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/polygenic_score_file_creator_PRScs/polygenic_score_file_creator_PRScs.R \
--ref_plink_chr ${Geno_1KG_dir}/1KGPhase3.w_hm3.chr \
--ref_keep ${Geno_1KG_dir}/keep_files/EUR_samples.keep \
--sumstats ${gwas_rep}/${gwas}.sumstats.gz \
--plink ${plink1_9} \
...
--PRScs_ref_path ${Geno_1KG_dir}/PRSs_LD_matrix/EUR \
--n_cores 6 \
...
sbatch -J SBayesR -n 5 --mem 35G -p brc,shared /users/k1806347/brc_scratch/Software/Rscript.sh /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Scripts/polygenic_score_file_creator_SBayesR/polygenic_score_file_creator_SBayesR.R \
--ref_plink ${Geno_1KG_dir}/1KGPhase3.w_hm3.GW \
--ref_freq_chr ${Geno_1KG_dir}/freq_files/EUR/1KGPhase3.w_hm3.EUR.chr \
--ref_keep ${Geno_1KG_dir}/keep_files/EUR_samples.keep \
--sumstats ${gwas_rep}/${gwas}.sumstats.gz \
