...
Geno_1KG_dir=$(sed -n '/Geno_1KG_dir/p' /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var | cut -d ' ' -f 2)
plink1_9=$(sed -n '/plink1_9/p' /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var | cut -d ' ' -f 2)
gwas_rep=$(sed -n '/gwas_rep/p' /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var | cut -d ' ' -f 2)
PRScs_dir=$(sed -n '/PRScs_dir/p' /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var | cut -d ' ' -f 2)
```
</details>
...
Pipeline_prep.var<-read.table('/users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var')
gwas_rep<-Pipeline_prep.var$V2[Pipeline_prep.var$V1 == 'gwas_rep']
...
plink1_9=$(sed -n '/plink1_9/p' /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var | cut -d ' ' -f 2)
gwas_rep=$(sed -n '/gwas_rep/p' /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var | cut -d ' ' -f 2)
FUSION_dir=$(sed -n '/FUSION_dir/p' /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var | cut -d ' ' -f 2)
TWAS_rep=$(sed -n '/TWAS_rep/p' /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var | cut -d ' ' -f 2)
```
</details>
...
Pipeline_prep.var<-read.table('/users/k1806347/brc_scratch/Software/MyGit/GenoPred/Variables_used/Pipeline_prep.var')
FUSION_dir<-Pipeline_prep.var$V2[Pipeline_prep.var$V1 == 'FUSION_dir']
TWAS_rep<-Pipeline_prep.var$V2[Pipeline_prep.var$V1 == 'TWAS_rep']
...
First FUSION SNP-weights must be converted into SCORE files for use in PLINK. This is done using the script called 'FUSION_score_file_creator.R' (info [here](https://github.com/opain/https://github.com/opain/GenoPred/tree/master/Scripts/FUSION_score_file_creator))

Then the score files are used to predict features in the 1000 Genomes reference and report the mean and SD. This is done using the script called 'FUSION_ref_scorer.R' (info [here](https://github.com/opain/https://github.com/opain/GenoPred/tree/master/Scripts/FUSION_ref_scorer))
...
This section uses an R script called 'Run_TWAS.R' for multiple GWAS as an array. Further information these scripts can be found [here](https://github.com/opain/GenoPred/tree/master/Scripts/Run_TWAS).
...
* Update all file paths to be variables
* Provide example variable file
* Create READMEs to explain Scripts called here
