...
if 'target_list' in config:
  rule impute_23andme_i:
    ...
    shell:
      "Rscript {workflow.basedir}/../Scripts/23andMe_imputer/23andMe_imputer.R \
        --FID {params.name} \
        --IID {params.name} \
        --geno {params.path} \
...
if 'target_list' in config:
  rule format_target_i:
    ...
    shell:
      "Rscript {workflow.basedir}/../Scripts/format_target/format_target.R \
...
rule ancestry_inference_i:
  ...
  shell:
    "rm -r -f {outdir}/{wildcards.name}/ancestry; \
     ...
     Rscript {workflow.basedir}/../Scripts/Ancestry_identifier/Ancestry_identifier.R \
...
checkpoint ancestry_reporter:
  ...
  conda:
    "../envs/analysis.yaml"
  shell:
    "Rscript {workflow.basedir}/../Scripts/pipeline_misc/ancestry_reporter.R {wildcards.name} {outdir} > {log} 2>&1"
...
rule outlier_detection_i:
  ...
  shell:
    "Rscript {workflow.basedir}/../Scripts/outlier_detection/outlier_detection.R \
      --target_plink_chr {outdir}/{wildcards.name}/geno/{wildcards.name}.ref.chr \
