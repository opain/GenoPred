def ancestry_munge(x, target_populations=None):
    """
    Process ancestry report and return list of populations target population filter.
    ...
    available_populations = available_populations + ['TRANS']
...
rule pc_projection_all:
  input:
    lambda w: expand(f"{outdir}/reference/target_checks/{{name}}/pc_projection-{{population}}.done", name=w.name, population = ancestry_munge(w.name, target_populations = config['target_populations']))
  output:
...
rule target_pgs_i:
  ...
  input:
    f"{outdir}/reference/target_checks/{{name}}/ancestry_reporter.done",
    lambda w: f"{outdir}/reference/target_checks/{{name}}/pc_projection-{{population}}.done" if 'continuous' in config["pgs_scaling"] else [],
  ...
  params:
    testing=config["testing"],
    discrete="T" if 'discrete' in config["pgs_scaling"] else "F",
  ...
  shell:
    "Rscript --vanilla ../Scripts/target_scoring/target_scoring_pipeline.R \
      ...
      --plink2 plink2 \
      --discrete_adjustment {params.discrete} \
      --continuous_adjustment {params.continuous} \
      --test {params.testing} \
      --n_cores {threads} > {log} 2>&1"

rule target_pgs_all:
  input:
    lambda w: expand(f"{outdir}/reference/target_checks/{{name}}/target_pgs-{{population}}.done", name=w.name, population = ancestry_munge(w.name, target_populations = config['target_populations']))
  output:
