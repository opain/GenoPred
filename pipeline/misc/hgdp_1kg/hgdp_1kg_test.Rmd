...
conf <- c(
  'outdir: /users/k1806347/oliverpainfel/Data/hgdp_1kg/GenoPred/sparse_test',
...
for(gwas in names(nomiss_pgs$EUR)){
  ...
  pred <- rbind(
    ...
    fread(
      paste0(
        '~/oliverpainfel/Data/hgdp_1kg/GenoPred/sparse_test/hgdp_1kg_miss/pgs/EUR/ptclump/',
        gwas, 
        '/hgdp_1kg_miss-',
        gwas,
        '-EUR.missingness')
...
cor(both_r2$r2, both_r2$rel_r2, use = 'p') 

library(ggplot2)
library(cowplot)

ggplot(both_r2, aes(x = rel_r2, y = r2)) +
...
# Very highly correlated, but the observed R2 is higher than the estimated.
