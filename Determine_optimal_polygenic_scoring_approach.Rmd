...
for(i in 1:length(gwas)){
  for(k in c('1KG','UKB','GCTB')){
    ...
    if(GCTB_hsq_all_i_k$Converge[1] == T){
      GCTB_hsq_all_i_k$Use[1]<-T
    } else {
      if(GCTB_hsq_all_i_k$Converge[2] == T){
        GCTB_hsq_all_i_k$Use[2]<-T
      }
    }
...
for(i in 1:length(pheno)){
  ...
  group='SBLUP'))
    ...
    if(GCTB_hsq_all_i_ref$Use[GCTB_hsq_all_i_ref$Process == 'Original'] == T){
      SBayesR_process<-''
...
for(i in 1:length(pheno)){
  ...
  group='SBLUP'))
    ...
    if(GCTB_hsq_all_i_ref$Use[GCTB_hsq_all_i_ref$Process == 'Original'] == T){
      SBayesR_process<-''
      pred_file<-rbind(pred_file,data.frame( predictors=paste0(dat_path,'SBayesR/',gwas[i],SBayesR_process,'/TEDS.w_hm3.',gwas[i],'.EUR.SBayesR_profiles'),
group='SBayesR'))
    } else {
      SBayesR_process<-'_P4'
      pred_file<-rbind(pred_file,data.frame( predictors=paste0(dat_path,'SBayesR/',gwas[i],SBayesR_process,'/TEDS.w_hm3.',gwas[i],'.EUR.SBayesR_profiles'),
...
for(i in 1:length(gwas)){
  ...
  res<-fread(paste0(Geno_1KG_dir,'/Score_files_for_polygenic/SBayesR/',gwas[i],'/GWAS_sumstats_SBayesR.GW.parRes'))
...
mod_res_all$HighPoly[mod_res_all$pi0_est < 0.98]<-T

# IBD, MultiScler, RheuArth, Prostate_Cancer, and Breast_Cancer are the low polygenicity outcomes.
...
# A pi0 of 0.98 identifies all phenotype in TEDS to high polygenicity, which is accurate.
mod_res_all$HighPoly<-F
mod_res_all$HighPoly[mod_res_all$pi0_est < 0.98]<-T
...
plot_A<-meta_res_eval_plots_indep[[1]] + labs(title=NULL)
plot_B<-meta_res_eval_diff_plots_indep[[1]] + labs(title=NULL)
...
for(file in 1:length(Files)){
    if(file == 1){
      ...
      gwas<-c('DEPR06','COLL01','BODY04','HEIG03','DIAB05','COAD01')
    } else {
      pheno<-c('IBD','MultiScler','RheuArth','Prostate_Cancer','Breast_Cancer')
      gwas<-c('CROH01','SCLE03','RHEU02','PRCA01','BRCA01')
    }
...
for(file in 1:length(Files)){
  ...
  for(i in 1:length(pheno)){
      ...
      if(file.exists(paste0('/users/k1806347/brc_scratch/Analyses/PRS_comparison/',Samples[file],'_outcomes_for_prediction/',pheno[i],'/Association_withPRSs/',Samples[file],'.w_hm3.UKBB_ref.',gwas[i],'_P4.EUR-PRSs_SBayesR.pred_eval.txt'))){
        ...
        res_eval_UKBB_P4_i$Model<-'P<0.4'
      ...
      if(file.exists(paste0('/users/k1806347/brc_scratch/Analyses/PRS_comparison/',Samples[file],'_outcomes_for_prediction/',pheno[i],'/Association_withPRSs/',Samples[file],'.w_hm3.',gwas[i],'_GCTBref_P4.EUR-PRSs_SBayesR.pred_eval.txt'))){
        ...
        res_eval_GCTB_P4_i$Model<-'P<0.4'
        res_eval_GCTB_P4_i$Ref<-'GCTB'
      }
          
      res_i<-do.call(rbind, list(res_eval_1KG_i,res_eval_1KG_P4_i,res_eval_UKBB_i,res_eval_UKBB_P4_i,res_eval_GCTB_i,res_eval_GCTB_P4_i))
  ...
  res$Model<-factor(res$Model, levels=c('Original','P<0.4'))
