...
opt$pheno<-'/users/k1806347/brc_scratch/Data/UKBB/Phenotype/PRS_comp_subset/UKBB.Depression.txt'
opt$keep<-'/users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/ukb18177_glanville_post_qc_id_list.UpdateIDs.fam'
opt$out<-'/users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/Depression/Association_withPRSs/UKBB.w_hm3.UKBB_ref.DEPR06.EUR-PRSs'
opt$n_core<-1
opt$compare_predictors<-T
opt$assoc<-T
opt$outcome_pop_prev<-0.15
opt$predictors<-'/users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/Depression/Association_withPRSs/UKBB.w_hm3.UKBB_ref.DEPR06.EUR-PRSs.predictor_groups'
...
#### pT + clump sparse (non-nested)
<details><summary>pT + clump PRSs: Sparse (non-nested)</summary>
```{bash, echo=T, eval=F}
##############################
# Evaluating predictive utility of pT + clump PRSs across multiple pTs individually and in combination
##############################
...
Modelling multiple PRSs based on a range of parameters consistenly provides an improvement over the best single PRS idenitfied by cross-validation.
...
for(i in c(1:9)){
  ...
  lassosum_res$Param[dim(lassosum_res)[1]]<-'Multi-PRS'
  lassosum_res$Model<-NULL
  lassosum_val<-read.table(paste0('/users/k1806347/brc_scratch/Data/1KG/Phase3/Score_files_for_poylygenic_lassosum/',gwas[i],'/UKBB.w_hm3.UKBB_ref.',gwas[i],'.log'), sep='&')
  lassosum_val_s<-as.numeric(gsub('s = ','',lassosum_val$V1[grepl('s = ',lassosum_val$V1)]))
  lassosum_val_lambda<-as.numeric(gsub('lambda =','',lassosum_val$V1[grepl('lambda =',lassosum_val$V1)]))
  lassosum_val_param<-paste0('s',lassosum_val_s,'.lambda',lassosum_val_lambda)
  lassosum_val_param<-substr(lassosum_val_param, 1, nchar(lassosum_val_param)-1) 
...
png('/mnt/lustre/users/k1806347/Analyses/TEDS_outcomes_for_prediction/PRS_method_comp_CrossVal_TEDS.UKBB_ref.png', units='px', res=300, width=3000, height=2750)
plot_grid(plotlist=plots, ncol = 2)
dev.off()
...
png('/mnt/lustre/users/k1806347/Analyses/TEDS_outcomes_for_prediction/PRS_method_comp_IndepVal_TEDS.UKBB_ref.png', units='px', res=300, width=3000, height=2750)
plot_grid(plotlist=plots, ncol = 2)
dev.off()
