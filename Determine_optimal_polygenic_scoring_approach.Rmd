---
title: "Determining Optimal Polygenic Scoring Method"
output: 
  html_document:
    toc: true
    theme: united
...
# Aims
1. Compare PRS derived from p-value and clumping method, and shrinkage methods (including lassosum and PRScs), using 10-fold cross validation to identify the optimal parameters.
...
opt$pheno<-'/users/k1806347/brc_scratch/Data/UKBB/Phenotype/Depression/ever_depressed_pheno_final.UpdateIDs.txt'
...
opt$out<-'/users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/Depression/Association_withPRSs/UKBB.w_hm3.DEPR06.EUR-PRSs_lassosum'
opt$n_core<-2
opt$compare_predictors<-T
opt$assoc<-T
opt$outcome_pop_prev<-0.15
opt$predictors<-'/users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/Depression/Association_withPRSs/UKBB.w_hm3.DEPR06.EUR-PRSs_lassosum.predictor_groups'
...
for(i in 1:4){
  ...
  Param_tmp<-sort(as.numeric(gsub('e.','e-',gsub('_group','',gsub(paste0(gwas[i],'_'),'',pT_clump$Model)))))
  pT_clump$Param<-as.character(as.numeric(gsub('e.','e-',gsub('_group','',gsub(paste0(gwas[i],'_'),'',pT_clump$Model)))))
...
cp /mnt/lustre/users/k1806347/Analyses/UKBB_outcomes_for_prediction/PRS_method_comp_CrossVal.png /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Images/Determine_optimal_polygenic_scoring_approach/

cp /mnt/lustre/users/k1806347/Analyses/UKBB_outcomes_for_prediction/PRS_method_comp_IndepVal.png /users/k1806347/brc_scratch/Software/MyGit/GenoPred/Images/Determine_optimal_polygenic_scoring_approach/
```

<center>

![Figure 1: Predictive utility estimated using 10-fold cross-validation](/users/k1806347/brc_scratch/Software/MyGit/GenoPred/Images/Determine_optimal_polygenic_scoring_approach/PRS_method_comp_CrossVal.png)

<br/>

***

![Figure 2: Predictive utility estimated using test set validation](/users/k1806347/brc_scratch/Software/MyGit/GenoPred/Images/Determine_optimal_polygenic_scoring_approach/PRS_method_comp_IndepVal.png)
...
res<-read.csv("/users/k1806347/brc_scratch/Analyses/UKBB_outcomes_for_prediction/PRS_method_comp.csv")
...
library(DT)
datatable(res, rownames = FALSE, filter="top", options = list(pageLength = 10, scrollX=T),caption='Table 1: Correlation between PRS model predictions and observed values')
```
