...
**Methodological caveat — quickprs AF-difference filter.** `Scripts/pgs_methods/quickprs.R:104-105` applies a likelihood-ratio test comparing GWAS vs reference allele frequencies and drops variants at `P < 1e-6`. For the mixed-ancestry mix_eur50 sumstats used here, this filter removes 436k–523k variants (30–38 % of ~1.4M) against every single-population LD reference — likely a driver of the quickprs × AMR convergence failure and part of the reason quickprs looks more LD-sensitive than dbslmm. Modifying the pipeline to relax or optionally disable this filter is a candidate future change so LD-mismatch and AF-mismatch effects can be isolated.
