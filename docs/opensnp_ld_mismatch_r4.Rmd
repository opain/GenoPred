...
**Methodological caveat — quickprs AF-difference filter.** `Scripts/pgs_methods/quickprs.R:104-105` applies a likelihood-ratio test comparing GWAS vs reference allele frequencies and drops variants at `P < 1e-6`. Under ancestry mismatch this filter is very aggressive (~30–40 % of variants dropped in prior rounds). Part of the "quickprs is more LD-sensitive than dbslmm — motivating Round 5" framing below is therefore contaminated by this AF-attrition effect, not purely a property of LD-aware shrinkage. Modifying the pipeline to relax or optionally disable this filter is a candidate future change so LD-mismatch and AF-mismatch effects can be isolated.
