...
# Save PGS into a single data from for each batch
## 1) Merge all GWAS PGS within each batch
merged_by_batch <- lapply(pgs, function(batch) {
  dt_list <- lapply(batch$TRANS, `[[`, "sbayesrc")
  Reduce(function(x, y)
    merge(x, y, by = c("FID", "IID"), all = TRUE, sort = FALSE),
    dt_list
  )
})

## (Optional but useful) add a batch indicator column
merged_by_batch <- Map(function(dt, nm) {
  dt[, batch := nm]
  # move batch to the front if you like
  setcolorder(dt, c("batch", "FID", "IID", setdiff(names(dt), c("batch", "FID", "IID"))))
  dt
}, merged_by_batch, names(merged_by_batch))

## 2) Bind all batches into one data.table
all_pgs <- rbindlist(merged_by_batch, use.names = TRUE, fill = TRUE)

write.table(all_pgs, '~/oliverpainfel/Data/miraclestrial/GenoPred/output/miracles_pgs.txt', col.names = T, row.names = F, quote = F)
...
## 2) Bind all batches into one data.table
all_pcs <- rbindlist(merged_by_batch, use.names = TRUE, fill = TRUE)

write.table(all_pcs, '~/oliverpainfel/Data/miraclestrial/GenoPred/output/miracles_pcs.txt', col.names = T, row.names = F, quote = F)
