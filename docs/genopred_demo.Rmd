...
## Step 2: Create 'base' conda environment

Conda is a software environment management system which is great way for easily downloading and storing software. We will use conda to create a 'base' environment that the GenoPred pipeline will run in.

<details>

<summary>Why is referred to as the 'base' environment?</summary>

It is referred to as the base environment as it is used to run the GenoPred pipeline, which in turn loads other conda environments to run specific analysis.
...
Once it is complete, you can then delete the `Miniconda3-latest-Linux-x86_64.sh1` file.

Now, go into the `pipeline` folder within the GenoPred repo, and create the conda environment based on the `envs/base.yaml` file. This will create an environment called `genopred`.
...
Note. Check out advice [here](#running-jobs-in-parallel) for running in parallel on an HPC. The command might take some time, so you might also consider submitting as a batch job or within a terminal multiplexer to avoid connection issues (see [here](#avoid-connection-issues)).
...
The format of the score files should be consistent with the PGS Catalogue header format (https://www.pgscatalog.org/downloads/#scoring_header).

- `rsID` - RSID
- `chr_name` - Chromosome number
- `chr_position` - Basepair position
...
GenoPred allows only one column of effect sizes per score file. It also doesn

GenoPred is lenient, and only requires either the RSIDs (`rsID`), or chromosome (`chr_name`) and base pair (`chr_position`) position columns to be present.
