...
for disc_pop in $(echo EUR EAS); do
  for targ_pop in $(echo EUR EAS); do
    ...
    for pheno in $(head -n 5 /scratch/prj/ukbiobank/usr/ollie_pain/phenotypes/prscsx/prscsx_labels.txt); do
      sbatch --mem 10G -n 1 -p cpu --wrap="Rscript ../Scripts/model_builder/model_builder.R \
...
for(pheno_i in prscsx_dat$labels){
  ...
  res_i$Group[!grepl('top1|PredFile', res_i$Model)]<-'Multi'
  
  res_i_pseudo<-res_i[grepl('PredFile', res_i$Model),]
  res_i<-res_i[!grepl('PredFile', res_i$Model),]
  ...
  for(pgs_method_i in pgs_methods[
    ...
    res_i_tmp<-res_i_pseudo[res_i_pseudo$Method == pgs_method_i,]
      ...
      for(gwas_i in gwas_list_i$name){
        ...
        res_i_tmp2 <-
          res_i_tmp[grepl(gwas_i_2, res_i_tmp$Model), ]
        res_i_tmp2$param<-gsub(paste0('.*', gwas_i_2, '.'), '', res_i_tmp2$Model)
  ...
  for(pgs_method_i in pgs_methods[
    ...
    pgs_methods %in% c('prscsx','xwing')]) { # Restrict to methods with pseudo method
      ...
      for(gwas_group_i in gwas_groups_i$name){
        ...
        res_i_tmp2 <-
          res_i_tmp[grepl(gwas_group_i_2, res_i_tmp$Model), ]
        res_i_tmp2$param<-gsub(paste0('.*', gwas_group_i_2, '.'), '', res_i_tmp2$Model)
