...
# Estimate SNP-h2 and rG across populations
...
gwas_list<-fread('/scratch/prj/ukbiobank/usr/ollie_pain/GenoPredPipe/usr/k1806347/configs/crosspop/gwas_list.txt')
...
for(pop in c('AFR','EAS', 'EUR')){
  for(pheno in prscsx_dat$labels){
    log <-
      readLines(
        paste0(
...
## Check PGS association direction

This is a sanity check to see whether the PGS are correlated with the outcome in the expected direction (positive). We will do this using the ptclump PGS and within population for simplicity (i.e. EUR -> EUR, etc)
...
# Restrict to ptclump
scores<-scores[grepl('ptclump', scores$method),]
...
ggplot(res_all, aes(x = Phenotype, y = BETA, fill = pT)) +
  ...
  geom_errorbar(aes(ymin=BETA-SE, ymax=BETA+SE), width=0, position=position_dodge(width = 0.7, preserve = "single")) +
  labs(y="BETA (SE)") +
  theme_half_open() +
  background_grid() +
  panel_border() +
