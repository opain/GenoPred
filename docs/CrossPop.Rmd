...
for targ_pop in $(echo EUR); do
  ...
  for disc_pop_i in ${disc_pop}; do
    #rm /users/k1806347/oliverpainfel/Analyses/crosspop/targ_${targ_pop}.disc_EUR_${disc_pop}/*/res*
...
tmp$Discovery_clean <- as.character(tmp$Discovery)
tmp$Discovery_clean[tmp$Discovery == 'EUR'] <- 'EUR GWAS'
tmp$Discovery_clean[tmp$Discovery == 'EAS'] <- 'EAS GWAS'
tmp$Discovery_clean[tmp$Discovery == 'AFR'] <- 'AFR GWAS'
tmp$Discovery_clean[tmp$Source == 'Multi'] <- 'Both'
tmp$Discovery_clean <- factor(tmp$Discovery_clean, 
                              levels = c(
                                         'EUR GWAS',
                                         'EAS GWAS',
...
ggplot(tmp, aes(x=label, y=R , fill = Model)) +
    ...
    geom_vline(xintercept = seq(1.5, length(unique(tmp$label))), linetype="dotted") +
    labs(y = "R (SE)", x='Method') +
    facet_grid(gwas_group ~ Discovery_clean, scales='free', space = 'free_x') +
    theme_half_open() +
    background_grid() + 
    panel_border() + 
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
ggplot(tmp[tmp$Discovery_clean %in% c('EUR GWAS','Both'),], aes(x=label, y=R , fill = Model)) +
    #geom_hline(yintercept = 0) +
    geom_errorbar(aes(ymin = R - SE, ymax = R + SE),
                  width = 0,
                  position = position_dodge(width = 1)) +
...
meta_tmp_simple$Model1<-factor(meta_tmp_simple$Model1, levels = c('IndivTrain', 'SumStatTrain', 'IndivTrain of SumStatTrain'))
