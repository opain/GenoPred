...
config<-c(
  ...
  "pgs_methods: ['ptclump','dbslmm','prscsx','xwing','tlprs','prscs','sbayesr','lassosum','ldpred2','megaprs']",
  "tlprs_methods: ['dbslmm','sbayesr','prscs']",
  "cores_prep_pgs: 20",
  "cores_target_pgs: 50"
)
    ...
    for(disc_pop_j in disc_pop){
      ...
      for(i in 1:nrow(scores_i_j)) {
        ...
        pgs <- fread(
          ...
          select = c('FID', 'IID', 
                     paste0(scores_i_j$name[i], '_', param)))
...
ggplot(meta_res, aes(x=Method, y=R , fill = Model)) +
    #geom_hline(yintercept = 0) +
