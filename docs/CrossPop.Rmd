...
# platelet count  has low sample size for AFR (N = 235)
mvp_afr_subset <- mvp_afr_subset[mvp_afr_subset$efoTraits != 'platelet count',]
...
```{bash}
for pheno in $(cat ~/oliverpainfel/Data/GWAS_sumstats/MVP/urls.txt | cut -d' ' -f 2); do
  url=$(awk -v var="$pheno" '$2 == var {print $1}' ~/oliverpainfel/Data/GWAS_sumstats/MVP/urls.txt)
