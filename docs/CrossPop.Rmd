  ...
  for(targ_pop_i in targ_pop){
    ...
    for(disc_pop_j in disc_pop){
      ...
      predictors_i<- do.call(rbind, list(
        scores_i_j_single_top1, scores_i_j_single_pseudo, scores_i_j_multi_single_pseudo 
      ))
...
for targ_pop in $(echo EAS AFR); do
  ...
  for disc_pop_i in ${disc_pop}; do
    for pheno in $(cat /users/k1806347/oliverpainfel/Analyses/crosspop/trait_subset.txt); do
      sbatch --mem 10G -n 5 -p neurohack_cpu --wrap="Rscript ../Scripts/model_builder/model_builder_top1.R \
...
tmp$label[grepl('Multi', tmp$Model)] <- paste0(tmp$label[grepl('Multi', tmp$Model)], '-Multi')
...
ggplot(tmp[tmp$Discovery_clean == 'Target-matched GWAS',], aes(x=label, y=R , fill = Model)) +
    #geom_hline(yintercept = 0) +
    geom_errorbar(aes(ymin = R - SE, ymax = R + SE),
                  width = 0,
                  position = position_dodge(width = 1)) +
...
ggplot(tmp[tmp$Discovery_clean == 'EUR GWAS',], aes(x=label, y=R , fill = Model)) +
    #geom_hline(yintercept = 0) +
    geom_errorbar(aes(ymin = R - SE, ymax = R + SE),
                  width = 0,
                  position = position_dodge(width = 1)) +
...
tmp_multi_pseudo <- tmp[tmp$Discovery_clean == 'Both',]
tmp_multi_pseudo <- tmp_multi_pseudo[!duplicated(tmp_multi_pseudo[, c('label','Target'), with=F]),]
tmp_multi_pseudo$Model <- 'IndivTrain'

ggplot(tmp_multi_pseudo, aes(x=label, y=R , fill = Model)) +
    #geom_hline(yintercept = 0) +
    geom_errorbar(aes(ymin = R - SE, ymax = R + SE),
                  width = 0,
                  position = position_dodge(width = 1)) +
...
tmp_crossval <- tmp[tmp$Model %in% c('IndivTrain','Multi-IndivTrain'),]
tmp_crossval$Model <- 'IndivTrain'

ggplot(tmp_crossval, aes(x=label, y=R , fill = Model)) +
    #geom_hline(yintercept = 0) +
    geom_errorbar(aes(ymin = R - SE, ymax = R + SE),
                  width = 0,
                  position = position_dodge(width = 1)) +
...
tmp_pseudoval <- tmp[tmp$Model %in% c('SumStatTrain'),]

ggplot(tmp_pseudoval, aes(x=label, y=R , fill = Model)) +
    #geom_hline(yintercept = 0) +
    geom_errorbar(aes(ymin = R - SE, ymax = R + SE),
                  width = 0,
                  position = position_dodge(width = 1)) +
