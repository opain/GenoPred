...
Here we will do the same thing, but include variants with MAF > 0.01 in any super population. We will restrict to chromosome 22 initially.
...
# Set up directories and variables
mkdir -p "/users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}"
outDir="/users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/${pop}"

# Create the merge list for PLINK
chr=22

# Convert to plink1 binary and subset EUR
/users/k1806347/oliverpainfel/Software/plink2_linux_avx2_20241011/plink2 \
  --pfile /users/k1806347/oliverpainfel/Software/MyGit/GenoPred/pipeline/resources/data/ref_dense/ref.chr${chr} \
  --make-bed \
  --mac 5 \
  --keep /users/k1806347/oliverpainfel/Software/MyGit/GenoPred/pipeline/resources/data/ref_dense/keep_files/${pop}.keep \
  --out /users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/ref.${pop}.chr${chr}

# Create ma_file.txt
echo -e "SNP\tA1\tA2" > /users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/ma_file.txt
cut -f 2,5,6 /users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/ref.${pop}.chr${chr}.bim >> /users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/ma_file.txt

# Define variables for R scripts
ma_file="/users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/ma_file.txt"
genotype="/users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/ref.${pop}.chr${chr}"
threads=1
tool="/users/k1806347/oliverpainfel/Software/gctb_2.5.4_Linux/gctb"
...
# Step 2: Submit the parallel jobs for LDstep2
# Note. I tried to submit this as batch jobs in parallel, but the job kept completing halfway through. Very odd. I think it might have been a memory issue.
seq 1 $(wc -l ${outDir}/ld.sh | cut -d ' ' -f 1) | parallel -j 1 'Rscript -e "SBayesRC::LDstep2(outDir=\"'$outDir'\", blockIndex={}, log2file=TRUE)"'

# Step 3: Eigen decomposition for each LD block (submit and wait for Step 2 to complete)
jobid_step3=$(sbatch --array=1-$(wc -l ${outDir}/ld.sh | cut -d ' ' -f 1) --parsable <<EOT
#!/bin/bash
#SBATCH --job-name=sbayesrc_eigen
#SBATCH --partition=neurohack_cpu
#SBATCH --cpus-per-task=${threads}
...
for pop in EUR; do
  ...
  rm $outDir/ld.sh
  rm -r $outDir/snplist
  rm /users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/ref.${pop}.GW*
  rm /users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/common/${pop}.log
  rm /users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/ma_file.txt
  rm /users/k1806347/oliverpainfel/Data/hgdp_1kg/sbayesrc_common/${pop}/mergelist.txt
done
