...
conf <- c(
  ...
  "pgs_methods: ['ptclump','megaprs','sbayesrc']"
)
...
# Create config file for dense configuration including ambiguous variants, but restricted to target variants
conf <- c(
  'outdir: /users/k1806347/oliverpainfel/Data/OpenSNP/GenoPred/dense_incl_ambig_restricted_test',
  'dense_reference: T',
  'keep_ambiguous: T',
  'restrict_to_target_variants: T',
  'config_file: misc/opensnp/config_dense_incl_ambig_restricted.yaml',
  'gwas_list: misc/opensnp/gwas_list_dense_test.txt',
  'score_list: misc/opensnp/score_list_dense_test.txt',
  'target_list: misc/opensnp/target_list.txt',
  "pgs_scaling: ['discrete', 'continuous']",
...
dir.create('misc/opensnp/', recursive = T)
dir.create('/users/k1806347/oliverpainfel/Data/OpenSNP/GenoPred', recursive = T)
write.table(conf, 'misc/opensnp/config_dense_incl_ambig_restricted.yaml', col.names = F, row.names = F, quote = F)
...
snakemake \
  --profile slurm \
  --use-conda \
  --configfile=misc/opensnp/config_sparse.yaml \
  output_all --retries 3 -n 
  
snakemake \
  --profile slurm \
  --use-conda \
  --configfile=misc/opensnp/config_sparse_restricted.yaml \
  --retries 3 output_all -n 
  
snakemake \
  --profile slurm \
  --use-conda \
  --configfile=misc/opensnp/config_dense.yaml output_all \
  --retries 3 -n 
  
snakemake \
  --profile slurm \
  --use-conda \
  --configfile=misc/opensnp/config_dense_incl_ambig.yaml \
  --retries 3 output_all -n 
  
snakemake \
  --profile slurm \
  --use-conda \
  --configfile=misc/opensnp/config_dense_incl_ambig_restricted.yaml \
  --retries 3 output_all -n 
...
Note. When using dense reference of common variants incl ambiguous variants, there are still ~10% missed when using the ExpressWeb weights. This appears to be due to score files containing rarer variants.
...
pgs <- list(
  ...
  dense_incl_ambig = read_pgs(config = 'misc/opensnp/config_dense_incl_ambig.yaml', name = 'opensnp')$opensnp,
  dense_incl_ambig_restricted = read_pgs(config = 'misc/opensnp/config_dense_incl_ambig_restricted.yaml', name = 'opensnp')$opensnp
)
...
for(test_i in unique(cor_eur$test)){
  for(pop_i in unique(cor_eur$pop)){
    for(gwas_i in unique(cor_eur$gwas[cor_eur$pop == pop_i])){
      for(pgs_method_i in unique(cor_eur$pgs_method[cor_eur$pop == pop_i & cor_eur$gwas == gwas_i & cor_eur$test == test_i])){
        ...
        # PseudoVal
        if(pgs_method_i %in% c('ptclump','sbayesr','sbayesrc','ldpred2','megaprs','prscs','lassosum','dbslmm','prscsx')){
...
cor_eur_subset$test <-
  factor(
    cor_eur_subset$test,
    levels = c('sparse', 'sparse_restricted', 'dense','dense_incl_ambig', 'dense_incl_ambig_restricted'),
    labels = c('HapMap3', "HapMap3\n(restricted)", "Dense", "Dense\n(incl. ambiguous)", "Dense\n(incl. ambiguous)\n(restricted)")
  )
