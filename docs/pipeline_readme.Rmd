...
![](Images/pipeline_readme/input_schematic_wide.png)
...
To run the pipeline with the test_data, we will use the `example_input/config_example.yaml`. It specifies some basic options and specifies the `target_list`, `gwas_list` and `score_list` in the `example_input` folder. The `testing` parameter is set to `chr22` so only chromosome 22.
...
config_file: example_input/config_example.yaml

# Specify location of gwas_list file
gwas_list: example_input/gwas_list_example.txt

# Specify location of target_list file
target_list: example_input/target_list_example.txt

# Specify location of score_list file
score_list: example_input/score_list_example.txt
...
target_list <- fread('../pipeline/example_input/target_list_example.txt')
...
gwas_list <- fread('../pipeline/example_input/gwas_list_example.txt')
...
score_list <- fread('../pipeline/example_input/score_list_example.txt')
...
### Step 4A: Triggering new outputs
...
# Save file
fwrite(gwas_list, '../pipeline/example_input/gwas_list_example.txt', quote = F, sep = ' ', na='NA')
```
...
### Step 4B: Modifying outputs

It is also possible to update the configuration of the pipeline, so that it changes the outputs, but doesn create new outputs. For example, if I were to change the `path` for a given GWAS in the `gwas_list`, then it wouldn't need to create additional output files, it would just need to update the outputs to be based on the new GWAS sumstats. Again, there is no problem with doing this as snakemake can detect when parameters have changed. Here is a demonstration.
...
# Use COAD01 path for BODY04 GWAS
gwas_list$path[1] <- gwas_list$path[2]
...
This time, the pipeline says there is nothing to be done, but highlights that parameters and inputs of certain rules have been changed. To list the files that need to be updated, it tells use we can use the `--list-input-changes` and `--list-params-changes` options.

```{bash}
snakemake --list-input-changes output_all
# test_data/output/test1/reference/gwas_sumstat/BODY04/BODY04-cleaned.gz

snakemake --list-params-changes output_all
# test_data/output/test1/reference/gwas_sumstat/BODY04/BODY04-cleaned.gz
```

In this case, the same output is affected by input and param changes, which is the QC'd version of the BODY04 GWAS. This is what we expect as we have changed the GWAS with the name BODY04.

To see what the pipeline will rerun, updating outputs with the new params, we can run the following: 

```{bash}
snakemake -n -R $(snakemake --list-input-changes output_all) output_all
```

The rules to be rerun are the same as when adding a new GWAS, but it will overwrite outputs for the BODY04 GWAS.
...
We can then run the pipeline to update the outputs as required:

```{bash}
snakemake -j1 -R $(snakemake --list-input-changes output_all) output_all
```
  ...
  type = list(
    ...
    reference = list(
      group = list(
        ...
        `PGS score file` = list(
          output = c(
            "{outdir}/reference/pgs_score_files/{method}/{gwas}/ref-{gwas}-EUR.scale"
          ),
          description = c(
