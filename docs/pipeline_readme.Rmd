...
We have made docker and singularity containers with GenoPred pre-installed. We provide an example of using the pipeline within the container [here](pipeline_23andMe.html). We also provide an example running the pipeline in an offline environment, using a container with pre-downloaded dependencies (see [here](#running-in-offline-environment)). The containers can avoid some installation issues, and allow the pipeline to be used on non-linux based systems.
