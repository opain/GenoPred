...
snakemake --profile slurm --use-conda --configfile=/scratch/prj/ukbiobank/usr/ollie_pain/GenoPredPipe/usr/k1806347/configs/benchmark/config.yaml output_all -n 
```
...
bm_files_i<-bm_files_i[
  ...
  !grepl('prep_pgs|target_pgs|sumstat_prep|outlier_detection', bm_files_i)]
...
sum(bm_dat_all$s)/60/60 # 215.3136 hours
sum(bm_dat_all$s)/60/60/50 # 4.306272 when allowing 50 jobs simultaneously
...
fig <- plot_ly(data, labels = ~category, values = ~values, type = 'pie',
               ...
               rotation = 270,  # This rotates the pie chart to move labels to the side
