...
snakemake --profile slurm --use-conda --configfile=/scratch/prj/ukbiobank/usr/ollie_pain/GenoPredPipe/usr/k1806347/configs/benchmark/config.yaml prep_pgs -n 

# Run outlier detection using dev version to allow relatedness estimation to be skipped
snakemake --profile slurm --use-conda --configfile=/scratch/prj/ukbiobank/usr/ollie_pain/GenoPredPipe/usr/k1806347/configs/benchmark/config.yaml outlier_detection -n -p
...
pheno_gwas<-data.frame(
  pheno=c(bin_phenos, con_phenos)
)
...
ggplot(cor_subset[cor_subset$pheno == 'bc' & cor_subset$pop %in% c('EUR','CSA'),], aes(x = pgs_method, y = r, fill = model)) +
  geom_errorbar(
    aes(ymin = r - se, ymax = r + se),
    width = .2,
    position = position_dodge(width = 0.7)
