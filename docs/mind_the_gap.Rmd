...
foreach(i = 1:nrow(jobs), .combine = c, .options.multicore = list(preschedule = FALSE)) %dopar% {
  ...
  rel_r2 <- rel_pgs_r2_missing_eigen_old(
    ld_dir = '~/oliverpainfel/Software/MyGit/GenoPred/pipeline/resources/data/sbayesrc_ref_dense/EUR',
    score_df = score_i,
    f_miss = call_rate, 
    metric = 'correlation')
...
# Currently, the estimated PGS R2 is lower than the observed. This may be due to estimation method assuming SNP weights correspond to standardised genotypes.
...
Combine the score files in advance and then run relative PGS R2 estimator.
...
score_list<-fread('~/oliverpainfel/Analyses/mind_the_gap/GenoPred/config/score_list_cad.txt')[1:20]
...
write.table(
  rel_r2, 
  paste0(
    '/users/k1806347/oliverpainfel/Analyses/mind_the_gap/GenoPred/output/reference/pgs_score_files/external/relative.rsq.txt'
  ), col.names = T, row.names = F, quote = F)
...
est_rel_r2 <- fread('/users/k1806347/oliverpainfel/Analyses/mind_the_gap/GenoPred/output/reference/pgs_score_files/external/relative.rsq.txt')
setnames(est_rel_r2, 'relative_R2', 'est_rel_r2')
est_rel_r2$rsq <- 0.6

est_rel_r2_orig <- readRDS('~/oliverpainfel/Analyses/mind_the_gap/sim/masking/target/pgs/masked/rsq_metrics_pgs_eigen.rds')
setnames(est_rel_r2_orig, 'rel_r2', 'est_rel_r2_orig')
setnames(est_rel_r2_orig, 'group', 'rsq')

both <- merge(obs_rel_r2, est_rel_r2, by.x = c('ID','rsq'), by.y = c('beta_set','rsq'))
both <- merge(both, est_rel_r2_orig, by = c('ID','rsq'))
both$rsq <- paste0('Masked RSQ < ', both$rsq)

ggplot(both, aes(x = est_rel_r2, y = est_rel_r2_orig)) +
  geom_abline(colour = 'red') +
  geom_point() +
  facet_wrap(. ~ rsq) +
  coord_fixed() +
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0,1)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "Estimated relative R2 (new)", y = "Estimated relative R2 (old)")

# They are identical

# Compare correlation-based R2 to variance based R2
both$var_based_rel_r2 <- both$sig / both$den

ggplot(both, aes(x = est_rel_r2, y = var_based_rel_r2)) +
  geom_abline(colour = 'red') +
  geom_point() +
  facet_wrap(. ~ rsq) +
  coord_fixed() +
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0,1)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "Estimated relative R2 (cor-based)", y = "Estimated relative R2 (var-based)")

ggplot(both, aes(x = var_based_rel_r2, y = obs_rel_r2)) +
  geom_abline(colour = 'red') +
  geom_point() +
  facet_wrap(. ~ rsq) +
  coord_fixed() +
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0,1)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "Estimated relative R2 (var-based)", y = "Observed relative R2")

ggplot(both, aes(x = est_rel_r2, y = obs_rel_r2)) +
  geom_abline(colour = 'red') +
  geom_point() +
  facet_wrap(. ~ rsq) +
  coord_fixed() +
  ...
  scale_x_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0,1)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "Estimated relative R2 (cor-based)", y = "Observed relative R2")
cor(rel_r2$var_based_rel_r2, rel_r2$relative_R2)
coef(summary(lm(rel_r2$var_based_rel_r2 ~ rel_r2$relative_R2)))
