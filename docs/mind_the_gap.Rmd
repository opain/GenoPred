...
for id in $(tail -n+2 ~/oliverpainfel/Analyses/mind_the_gap/GenoPred/config/score_list_cad.txt | cut -f 1 -d ' '); do
  ...
  plink2 \
    ...
    --score ~/oliverpainfel/Analyses/mind_the_gap/GenoPred/output/reference/pgs_score_files/external/${id}/ref-${id}.score.nonzero.gz 1 2 4 header cols=+scoresums \
...
freq <- freq[, c('SNP','REF','ALT','ALT_FREQS'), with = F]
...
for(i in score_list$name){
  ...
  setnames(freq, "ID", "SNP")
  dt <- merge(score, freq[, .(SNP, A1f=A1, A2f=A2, FREQ)], by="SNP")
  dt <- merge(dt, rsqtab_subset[, .(SNP, call=pmin(pmax(call,0),1))], by="SNP", all.x=TRUE)
  dt[is.na(call), call := 1]
  
  # flip to target allele coding if needed
  flip <- dt$A1 != dt$A1f & dt$A1 == dt$A2f
  dt[SCORE_external := ifelse(flip, -SCORE_external, SCORE_external)][, A1 := A1f][]
  
  # allele SD and “true” weights w_j = S_j * BETA_j (used for noise variance)
  dt[, S := sqrt(2 * pmin(MAF, 1-FREQ) * (1 - pmin(FREQ, 1-FREQ)))]
  dt[, w_true := S * BETA]
  
  # attenuated weights for the signal term
  dt[, BETA_att := sqrt(call) * BETA]
  fwrite(dt[, .(SNP, A1, BETA_att)], "attenuated_weights.txt", sep="\t")
}
