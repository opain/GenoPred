...
for(i in score_list$name){
  ...
  n_flip <- log[grepl('variants were flipped to match reference.', log)]
  n_flip <- gsub(' variants were flipped to match reference.', '', n_flip)
...
Lets check what proportion of non-ambiguous variants are on the forward strand in PGS score files. This will help determine whether we can safely assume strand ambiguous variants are on the forward strand as well, and their use for downstream polygenic scoring is reliable. Probably easiest to do this with the GenoPred external_score processor script.
