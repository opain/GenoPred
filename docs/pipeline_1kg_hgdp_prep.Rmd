...
```{bash}
# 1KG+HGDP data
cd /scratch/users/k19054740/hgdp_1kg
wget https://ftp.ebi.ac.uk/pub/databases/spot/pgs/resources/pgsc_HGDP+1kGP_v1.tar.zst #This is ~15GB
...
# We need builds 37 and 39 POS for all these SNPs
chmod +x /scratch/prj/sgdp_strdep/maria/genetic/liftOver
# For b36 (hg18) to b37 (hg19)
wget http://hgdownload.soe.ucsc.edu/goldenPath/hg18/liftOver/hg18ToHg19.over.chain.gz
# For b37 (hg19) to b39 (hg38)
wget http://hgdownload.soe.ucsc.edu/goldenPath/hg19/liftOver/hg19ToHg38.over.chain.gz
# Make .bed for X
awk 'BEGIN{OFS="\t"} $1==23 {print "chr23", $4-1, $4, $2}' hapmap3_r1_b36_fwd_consensus.qc.poly.recode.map > chr23.b36.bed
sed 's/^chr23/chrX/' chr23.b36.bed > chrX.b36.bed
# Make corresponding new .ped for X
module load plink/1.9-beta6.10-gcc-13.2.0
plink --file hapmap3_r1_b36_fwd_consensus.qc.poly.recode --chr X --recode --out chrX_only

#Run liftOver x2
/scratch/prj/sgdp_strdep/maria/genetic/liftOver chrX.b36.bed hg18ToHg19.over.chain.gz chr23.b37.bed unMapped.b36to37.txt
/scratch/prj/sgdp_strdep/maria/genetic/liftOver chr23.b37.bed hg19ToHg38.over.chain.gz chr23.b39.bed unMapped.b37to39.txt

***

# Convert to .rds to match ref files for 1-22
```{r}
# Read X .bed (SNP order & info)
map <- read.table("/scratch/users/k19054740/hgdp_1kg/chrX.b36.bed", stringsAsFactors=FALSE, header=FALSE)
colnames(map) <- c("CHR", "GenDist", "BP_GRCh36", "SNP")
map$CHR <- ifelse(map$CHR == "chrX", "23", map$CHR)

# Read .ped (genotypes start at column 7)
ped <- read.table("/scratch/users/k19054740/hgdp_1kg/chrX_only.ped", stringsAsFactors=FALSE, header=FALSE)
# Extract alleles from original hm3 .ped (build36)
allele_cols <- as.character(unlist(ped[1, 7:ncol(ped)]))
A1 <- allele_cols[seq(1, length(allele_cols), by=2)]
A2 <- allele_cols[seq(2, length(allele_cols), by=2)]
alleles_df <- data.frame(SNP = map$SNP, A1 = A1, A2 = A2, stringsAsFactors = FALSE)

# Read build 36, 37 and 39 .bed files and get positions
read_bed <- function(file) {
  bed <- read.table(file, stringsAsFactors = FALSE, header = FALSE)
  colnames(bed) <- c("CHR", "start", "end", "SNP")
  bed$BP <- bed$start + 1
  return(bed[, c("SNP", "CHR", "BP")])
}

b36 <- read_bed("/scratch/users/k19054740/hgdp_1kg/chrX.b36.bed")
b37 <- read_bed("/scratch/users/k19054740/hgdp_1kg/chr23.b37.bed")
b38 <- read_bed("/scratch/users/k19054740/hgdp_1kg/chr23.b39.bed")

colnames(b36)[3] <- "BP_GRCh36"
colnames(b37)[3] <- "BP_GRCh37"
colnames(b38)[3] <- "BP_GRCh38"

# Merge all POS by SNP
pos_df <- merge(b36[, c("SNP", "BP_GRCh36")], 
                b37[, c("SNP", "BP_GRCh37")], by = "SNP", all = TRUE)
pos_df <- merge(pos_df, b38[, c("SNP", "BP_GRCh38")], by = "SNP", all = TRUE)

# Merge alleles with positions
final_df <- merge(pos_df, alleles_df, by = "SNP", all.x = TRUE)
# Add chromosome info from original .map
final_df$CHR <- map$CHR[match(final_df$SNP, map$SNP)]

# Reorder columns
final_df <- final_df[, c("CHR", "SNP", "BP_GRCh36", "BP_GRCh37", "BP_GRCh38", "A1", "A2")]

#Write
saveRDS(final_df, "/scratch/users/k19054740/software/GenoPred/pipeline/resources/data/ref/ref.chr23.rds")
```
...
```{r}
setwd('/scratch/users/k19054740/software/GenoPred/pipeline')
...
pvar <- fread('/scratch/users/k19054740/hgdp_1kg/GRCh37_HGDP+1kGP_ALL.pvar', nThread=5)

# Remove unneeded data and variants that aren't SNPs
pvar$INFO<-NULL
pvar$FILTER<-NULL
pvar<-pvar[nchar(pvar$REF) == 1,]
pvar<-pvar[nchar(pvar$ALT) == 1,]
names(pvar)<-c('CHR','BP','SNP','A1','A2')
pvar$CHR[pvar$CHR == "X"] <- "23"
pvar<-pvar[pvar$CHR %in% 1:23,]
pvar$CHR<-as.numeric(pvar$CHR)

# Read in the hapmap snp data
# Use the previously prepared 1KG reference data for X 
hm3<-NULL
for(i in 23){
  hm3<-rbind(hm3, readRDS(paste0('/scratch/users/k19054740/software/GenoPred/pipeline/resources/data/ref/ref.chr',i,'.rds')))
}

hm3<-hm3[,c('CHR','SNP','BP_GRCh36','BP_GRCh37','BP_GRCh38','A1','A2')]
names(hm3) <- c('CHR','SNP','BP_GRCh36','BP','BP_GRCh38','A1','A2')
hm3$CHR<-as.numeric(hm3$CHR)
...
sum(flip) # 16 here not 0 

# Retain variants that match IUPAC
pvar_hm3<-pvar_hm3[pvar_hm3$IUPAC.x == pvar_hm3$IUPAC.y,]

# Save intermediate file
write.table(pvar_hm3,'/scratch/users/k19054740/hgdp_1kg/hgdp_1kg_hm3_snps.txt', col.names = T, row.names = F, quote = F)

# Extract hm3 SNPs from plink2 files
dir.create('/scratch/users/k19054740/hgdp_1kg/genopred')
write.table(pvar_hm3$SNP.x, '/scratch/users/k19054740/hgdp_1kg/genopred/extract.snplist', col.names=F, row.names=F, quote=F)
system('plink2 --pfile /scratch/users/k19054740/hgdp_1kg/GRCh37_HGDP+1kGP_ALL --make-pgen --extract /scratch/users/k19054740/hgdp_1kg/genopred/extract.snplist --out /scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3 --threads 5')

# Insert RSIDs into new plink files
pvar <- fread('/scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3.pvar')
...
fwrite(pvar, '/scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3.pvar', col.names=T, row.names=F, quote=F, sep=' ')

# Remove hard filtered individuals
samp_meta<-fread('/scratch/users/k19054740/hgdp_1kg/gnomad.genomes.v3.1.2.hgdp_1kg_subset_sample_meta.tsv.bgz')
...
psam <- fread('/scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3.psam')
...
fwrite(relations, file='/scratch/users/k19054740/hgdp_1kg/genopred/relations.txt', sep=' ', quote=F)

system('/scratch/prj/ukbiobank/KCL_Data/Software/tools/GreedyRelated-master-v1.2.1/GreedyRelated -r /scratch/users/k19054740/hgdp_1kg/genopred/relations.txt -t 0.05 -s 1 > /scratch/users/k19054740/hgdp_1kg/genopred/relations_remove.txt')

remove<-fread('/scratch/users/k19054740/hgdp_1kg/genopred/relations_remove.txt')
...
write.table(psam_unrel[, '#IID', with=F], '/scratch/users/k19054740/hgdp_1kg/genopred/unrel.keep', col.names=T, row.names=F, quote=F)

# Split by chromosome and retain unrelated individuals
for(i in 1:23){
system(paste0('plink2 --pfile /scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3 --keep /scratch/users/k19054740/hgdp_1kg/genopred/unrel.keep --chr ',i,' --make-pgen --out /scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3.chr',i))
}

# Create keep files and calculate allele frequencies for each population
# Population data is stored withint the psam file
psam <- fread('/scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3.chr22.psam')
pop_dat<-psam[, c('#IID','SuperPop')]
names(pop_dat)<-c('#IID','POP')
write.table(pop_dat, '/scratch/users/k19054740/hgdp_1kg/genopred/pop.txt', row.names=F, col.names=T, quote=F)

dir.create('/scratch/users/k19054740/hgdp_1kg/genopred/keep_files/')
for(pop in unique(pop_dat$POP)){
  write.table(pop_dat[pop_dat$POP == pop, '#IID', with=F], paste0('/scratch/users/k19054740/hgdp_1kg/genopred/keep_files/',pop,'.keep'), col.names=F, row.names=F, quote=F)
  dir.create(paste0('/scratch/users/k19054740/hgdp_1kg/genopred/freq_files/', pop), recursive = T)
  for(i in 23){
    system(paste0('plink2 --pfile /scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3.chr',i,' --keep /scratch/users/k19054740/hgdp_1kg/genopred/keep_files/', pop, '.keep --chr ',i, ' --freq --out /scratch/users/k19054740/hgdp_1kg/genopred/freq_files/', pop, "/hgdp_1kg_hm3.chr", i))
  }
}

# Create frequency files across all reference individuals (TRANS)
dir.create(paste0('/scratch/users/k19054740/hgdp_1kg/genopred/freq_files/TRANS'), recursive = T)
for(i in 23){
  system(paste0('plink2 --pfile /scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3.chr',i,' --chr ',i,' --freq --out /scratch/users/k19054740/hgdp_1kg/genopred/freq_files/TRANS/hgdp_1kg_hm3.chr', i))
}
...
for(chr in 1:23){
    ...
    for(pop in unique(pop_dat$POP)){
        ...
        freq<-fread(paste0('/scratch/users/k19054740/hgdp_1kg/genopred/freq_files/', pop,'/hgdp_1kg_hm3.chr', chr,'.afreq'))
    ...
    saveRDS(ref, file = paste0('/scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3.chr',chr,'.rds'))
}
...
write.table(keep_list, '/scratch/users/k19054740/hgdp_1kg/genopred/ref.keep.list', col.names=F, row.names=F, quote=F)
...
# Copy over all the relevant files to main dir
mkdir scratch/users/k19054740/hgdp_1kg/genopred/ref

for chr in $(seq 1 23); do
for file in $(echo pgen pvar psam rds); do
    cp /scratch/users/k19054740/hgdp_1kg/genopred/hgdp_1kg_hm3.chr23.${file} /scratch/users/k19054740/hgdp_1kg/genopred/ref/ref.chr23.${file}
done
...
mkdir -p /users/k1806347/k19054740/ld_scores
...
pvar<-pvar[nchar(pvar$REF) == 1,]
pvar<-pvar[nchar(pvar$ALT) == 1,]
names(pvar)<-c('CHR','BP','SNP','A1','A2') #Note: no rsid here
pvar$CHR[pvar$CHR == "X"] <- "23"
pvar$CHR<-as.numeric(pvar$CHR)
pvar<-pvar[pvar$CHR %in% 1:23,]
