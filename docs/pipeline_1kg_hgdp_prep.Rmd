...
### Unambiguous and biallelic

Make version containing only unambiguous biallelic variants.
...
Make version containing unambiguous and ambiguous SNPs.
...
dir.create('/users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig')
...
# Read in dbSNP data and filter in the same way
dbsnp<-fread('~/oliverpainfel/Data/dbSNP/00-common_all.vcf.mini.split', header=F, nThread=5)
names(dbsnp)<-c('CHR','BP','SNP','A1','A2')
dbsnp<-dbsnp[dbsnp$CHR %in% 1:22,]
dbsnp$CHR<-as.numeric(dbsnp$CHR)
dbsnp$IUPAC<-snp_iupac(dbsnp$A1, dbsnp$A2)
...
pvar$in_hm3<-paste0(pvar$CHR,':', pvar$BP, ':', pvar$IUPAC) %in% paste0(hapmap3_ref$CHR,':',hapmap3_ref$BP,':',hapmap3_ref$IUPAC)
sum(pvar$in_hm3) # 1204057 of 1204449. This is excellent coverage

# Check overlap with SBayesRC reference
sbayesrc_ref <- fread('~/oliverpainfel/Data/SBayesRC/ukbEUR_Imputed/snp.info')
sbayesrc_ref <- sbayesrc_ref[, c('Chrom','PhysPos','ID','A1','A2'), with=F]
names(sbayesrc_ref) <- c('CHR','BP','SNP','A1','A2')
sbayesrc_ref$IUPAC <- snp_iupac(sbayesrc_ref$A1, sbayesrc_ref$A2)

pvar$in_sbayesrc<-paste0(pvar$CHR,':', pvar$BP, ':', pvar$IUPAC) %in% paste0(sbayesrc_ref$CHR,':', sbayesrc_ref$BP, ':',sbayesrc_ref$IUPAC)
sum(pvar$in_sbayesrc) # 7345762 of 7356518. This is excellent coverage

# See whether hm3 and sbayesrc snplists contain multi-allele variants
# Neither contain duplicate RSIDs
sum(duplicated(pvar$SNP[pvar$SNP %in% pvar$SNP[pvar$in_sbayesrc]])) # 8642
sum(duplicated(pvar$SNP[pvar$SNP %in% pvar$SNP[pvar$in_sbayesrc]])) # 58260
# Yes, both contain multi-allelic variants
# We should retain the alleles of these variants that are in hm3 and sbayesrc

pvar <- pvar[rev(order(pvar$in_hm3, pvar$in_sbayesrc)),]
nrow(pvar) # 32026836
pvar <- pvar[!duplicated(pvar$SNP),]
nrow(pvar) # 31592915

sum(pvar$in_hm3) # 1204057
sum(pvar$in_sbayesrc) # 7345762
# Perfect. We have remove multi allelic variants without loosing variants from hm3 or sbayesrc snplists.

pvar<-pvar[, c('CHR','BP','ID','SNP','A1','A2'), with=F]

# Extract SNPs from plink2 files
write.table(pvar$ID, '/users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/extract.snplist', col.names=F, row.names=F, quote=F)

# Save intermediate file
fwrite(pvar,'/users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/hgdp_1kg_nonambig_snps.txt', sep = '\t', quote = F, na='NA')
...
for chr in $(seq 1 22); do
  sbatch --mem 10G -n 5 -p neurohack_cpu -t 1:00:00 --wrap="plink2 \
    --pfile /users/k1806347/oliverpainfel/Data/hgdp_1kg/GRCh37_HGDP+1kGP_ALL \
    --keep /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred/unrel.keep \
    --extract /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/extract.snplist \
    --chr ${chr} \
    --threads 5 \
    --make-pgen \
    --out /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/hgdp_1kg.chr${chr}"
done
```
...
# Insert RSIDs into new plink files
ref <- fread('/users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/hgdp_1kg_nonambig_snps.txt', nThread=5)

for(i in 1:22){
  print(i)
  pvar <- fread(paste0('/users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/hgdp_1kg.chr',i,'.pvar'), nThread=5)
  pvar <- pvar[, c('#CHROM','POS','ID','REF','ALT'), with=F]
  pvar[ref, on=.(ID), ID := i.SNP]
  fwrite(
    pvar,
    paste0('/users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/hgdp_1kg.chr',i,'.pvar'),
    col.names = T,
    row.names = F,
    quote = F,
    sep = ' ',
...
# Use keep files from previous reference preparation
for pop in $(echo EUR EAS AFR AMR CSA MID); do
  mkdir -p /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/freq_files/${pop}
  
  for chr in $(seq 1 22); do
    sbatch --mem 10G -n 1 -p neurohack_cpu -t 1:00:00 --wrap="plink2 \
      --pfile /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/hgdp_1kg.chr${chr} \
      --keep /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred/keep_files/${pop}.keep \
      --freq \
      --out /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/freq_files/${pop}/hgdp_1kg.chr${chr}"
      
  done
done

# Create frequency files across all reference individuals (TRANS)
mkdir -p /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/freq_files/TRANS
for chr in $(seq 1 22); do
  sbatch --mem 10G -n 1 -p neurohack_cpu -t 1:00:00 --wrap="plink2 \
    --pfile /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/hgdp_1kg.chr${chr} \
    --freq \
    --out /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/freq_files/TRANS/hgdp_1kg.chr${chr}"
done
```
...
for(i in 1:22){
  ...
  for(pop in c('EUR','EAS','AFR','AMR','CSA','MID')){
    freq <-
      fread(
        paste0(
          '/users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/freq_files/',
          pop,
          '/hgdp_1kg.chr',
          i,
          '.afreq'
  ...
  saveRDS(pvar, file = paste0('/users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/hgdp_1kg.chr', i,'.rds'))
}
...
# Copy over all the relavent files
mkdir /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref

for chr in $(seq 1 22); do
  for file in $(echo pgen pvar psam rds); do
    cp /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/hgdp_1kg.chr${chr}.${file} /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref/ref.chr${chr}.${file}
  done
done

cp /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred/pop.txt /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref/ref.pop.txt

mkdir /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref/keep_files

cp /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred/keep_files/*.keep /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref/keep_files/

cp -r /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/freq_files /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref/

for pop in $(ls /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref/freq_files); do
 for chr in $(seq 1 22); do
  mv /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref/freq_files/${pop}/hgdp_1kg.chr${chr}.afreq /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref/freq_files/${pop}/ref.${pop}.chr${chr}.afreq
  rm /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig/ref/freq_files/${pop}/hgdp_1kg.chr${chr}.log
 done
done

# Compress folder and upload to google drive
cd /users/k1806347/oliverpainfel/Data/hgdp_1kg/genopred_dense_incl_ambig
tar -czvf genopred_dense_incl_ambig_1kg_hgdp.tar.gz ref
~/oliverpainfel/Software/gdrive files upload genopred_dense_incl_ambig_1kg_hgdp.tar.gz
```
...
Now we are not going to filter out strand ambiguous variants.
...
bcftools norm -m -any --threads 10 -Ou ~/oliverpainfel/Data/dbSNP/00-All.vcf.gz \
  | bcftools query -f '%CHROM\t%POS\t%ID\t%REF\t%ALT\n' \
  > ~/oliverpainfel/Data/dbSNP/00-All.split.tsv
