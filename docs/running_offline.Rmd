...
The GenoPred image has already been built using the Dockerfile [here](https://github.com/opain/GenoPred/blob/master/pipeline/misc/docker/Dockerfile). This image can be pulled using either docker or singularity.

```{bash}
# Go to directory where you want to save the container image
cd /users/k1806347/oliverpainfel/Software/singularity

# Docker
docker pull opaino/genopred_pipeline:v0.1

# Singularity
singularity pull docker://opaino/genopred_pipeline:v0.1
```
...
singularity shell \
  --contain \
  --home /users/k1806347/oliverpainfel:/users/k1806347/oliverpainfel \
...
echo "resdir: /users/k1806347/oliverpainfel/test/offline_example/genopred_resources" >> example_input/config.yaml
