...
### Download precomputed height score file from PGS catalogue

<details><summary>Show code</summary>

```{bash}
# These are from EUR Yengo 2022 (incl. 23ndMe) - The GWAS sumstats used by GenoPred do not incl. 23andMe
wget --no-check-certificate -O /users/k1806347/oliverpainfel/Data/GWAS_sumstats/opensnp_test/yengo_2022_height_eur_pgs_cat.gz https://ftp.ebi.ac.uk/pub/databases/spot/pgs/scores/PGS002804/ScoringFiles/Harmonized/PGS002804_hmPOS_GRCh37.txt.gz
...
# Create config file
conf <- c(
  'outdir: /users/k1806347/oliverpainfel/Data/OpenSNP/GenoPred/test1',
...
score_list <- data.frame(
  ...
  path = '/users/k1806347/oliverpainfel/Data/GWAS_sumstats/opensnp_test/yengo_2022_height_eur_pgs_cat.gz',
...
plot_obj_ext <- 
  ggplot(cor_eur_subset[cor_eur_subset$gwas == 'yengo_eur' | cor_eur_subset$gwas == 'height_external',], aes(x = pgs_method, y = r, fill = model)) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single"), width = 0.7) +
  geom_errorbar(
    aes(ymin = r - se, ymax = r + se),
    width = .2,
